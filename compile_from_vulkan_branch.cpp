...
#define PUSH_IMM(i) \
	...
	case IR_CONTINUE:
	{
		...
		depth--;
		/*
		if (aux && aux->ir && aux->ir->block.is_for_loop)
		{
...
struct vulkan_graphics_pipeline {
  ...
  VkPipeline pipeline;
  own_std::vector<VkDescriptorSet> descriptor_sets;
};
struct shader_info
{
...
own_std::string WasmGetStack(dbg_state* dbg)
{
	own_std::string ret = "stack: \n";
	char buffer[512];
	int i = 0;
...
own_std::string WasmGenRegStr(dbg_state* dbg, int reg, int reg_sz, own_std::string reg_name)
{
	unsigned int reg_val = WasmGetMemOffsetVal(dbg, reg);
	char buffer[64];
	own_std::string reg_idx = WasmNumToString(dbg, reg);
	own_std::string reg_val_str = WasmNumToString(dbg, reg_val);
	own_std::string ptr_val_str;
	own_std::string reg_size_str;
	...
	if (reg_sz != 0)
		reg_size_str = own_std::to_string(reg_sz);
...
own_std::string WasmGetSingleExprToStr(dbg_state* dbg, dbg_expr* exp)
{
	own_std::string ret = "expression: " + exp->exp_str;

	//if(exp->)
	typed_stack_val expr_val;
	WasmFromAstArrToStackVal(dbg, exp->expr, &expr_val);
	char buffer[512];
	own_std::string addr_str = WasmNumToString(dbg, expr_val.offset);
	...
	if (expr_val.type.type == TYPE_STRUCT)
	{
		...
		//own_std::string WasmVarToString(dbg_state* dbg, char indent, decl2* d, int struct_offset)
		...
		own_std::string struct_str = WasmVarToString(dbg, 0, d, expr_val.offset, ptr);
...
*/
command_info* GetSuggestion(command_info *cur, own_std::string incomplete_str)
{
...
void InsertSuggestion(dbg_state* dbg, own_std::string &input, int *cursor_pos)
{
	...
	FOR_VEC(t, tkns)
	{
		...
		if (cur->end)
		{
			...
			if (cur->func)
			{
				...
				ASSERT(cur->func);
				own_std::string final_str = cur->func(dbg, &args);
		...
		{
			input.erase(t->line_offset, t->str.size());
			own_std::string final_str = c->names[0];
...
bool WasmBcLogic(wasm_interp* winterp, dbg_state& dbg, wasm_bc** cur_bc, unsigned char* mem_buffer, block_linked** cur, bool &can_break)
{
	...
	case WASM_INST_CALL:
	{
		...
		if (IS_FLAG_ON(call_f->flags, FUNC_DECL_IS_OUTSIDER))
		{
			OutsiderFuncType found = winterp->get_func(call_f->name);
	...
	*(u64*)&dbg->mem_buffer[PRE_X64_RSP_REG * 8] = STACK_PTR_START;
    ...
    FOR_VEC(m, dbg->mem_watches)
    {
...
#ifdef RENDERER_VULKAN
      int imageIndex = BeginDebugVulkanDraw(dbg);
      //ImGui_ImplVulkan_NewFrame();
      //ImGui_Impln_NewFrame();
      ...
      //HERE()
			...
			// Rendering
#ifdef RENDERER_VULKAN
      EndDebugVulkanDraw(dbg, imageIndex);
...
own_std::string WasmCmdPrintWasmFuncAutoComplete(dbg_state* dbg, command_info_args *info)
{
	own_std::string ret = "";

	own_std::vector<own_std::string> names_found;
		
	FOR_VEC(func, dbg->lang_stat->winterp->funcs)
	{
		own_std::string n = (*func)->name;
		bool is_equal = true;
	...
	own_std::string shortest_str = names_found[shortest_str_idx];
	own_std::string second_shortest_str = names_found[second_shortest_str_idx];
...
void WasmPushNameIntoArray(own_std::vector<unsigned char>* out, own_std::string name)
{
	own_std::vector<unsigned char> uleb;
...
void GenX64ToIrValDecl2(lang_state *lang_stat, own_std::vector<byte_code>& ret, ir_val_aux *aux, ir_val *ir, bool address, bool if_its_packed_reg_mov_extended, short reg_dst = -1)
{
	...
	if (!address && if_its_packed_reg_mov_extended && 
		aux->type == IR_TYPE_REG && aux->is_float && 
		!aux->is_packed_float && (aux->deref == -1 || aux->deref != -1 && aux->reg != PRE_X64_RSP_REG))
	{
		byte_code bc;
		bc.type = FILL_SSE_2_PCKED_SSE;
		bc.bin.rhs.reg = aux->reg;
	...
	assign_info &assign, int line)
{
	...
	if (assign.only_lhs)
	{
		...
		case IR_TYPE_REG:
		{
			...
			case IR_TYPE_DECL:
			{
				...
				if (reg_dst_deref < 0)
				{
					byte_code_enum inst = MOV_SSE_2_SSE;
					...
					GenX64RegToReg(lang_stat, ret, reg_dst, reg_dst_sz, lhs.reg, inst);
				}
				else
				{
					byte_code_enum inst;
					if (assign.to_assign.is_float)
					{
						...
						inst = STORE_R_2_M;
					GenX64RegToMem(ret, lhs.reg, lhs.reg_sz, 0, reg_dst, inst);
				}
	...
	else
	{
		...
		{
			ir_val_aux lhs = {};
			if  (assign.lhs.type == IR_TYPE_REG &&assign.lhs.is_packed_float && assign.lhs.ptr <= 0)
			{
				GenX64ToIrValReg2(lang_stat, ret, &lhs, &assign.lhs, false, true, assign.lhs.reg);
				...
				case 4:
				{
					bc.shuffle.mask = _MM_SHUFFLE(0, 0, 0, 1);
				}break;
				// z
				case 8:
				{
					bc.shuffle.mask = _MM_SHUFFLE(0, 0, 0, 2);
				}break;
				// w
				case 12:
				{
					bc.shuffle.mask = _MM_SHUFFLE(0, 0, 0, 3);
				}break;
				default:
					ASSERT(0);
				}
			...
			else
			{
				...
				case IR_TYPE_ON_STACK:
				{
					GenX64ToIrValDecl2(lang_stat, ret, &lhs, &assign.lhs, true, false);
				}break;
				case IR_TYPE_DECL:
				{
					short reg = PRE_X64_RSP_REG;
			...
			case IR_TYPE_REG:
			{
				...
				GenX64ToIrValReg2(lang_stat, ret, &dst, &assign.to_assign, false, false, -1);
				byte_code_enum inst;
				if (dst.deref > 0)
					if (lhs.is_float)
						inst = MOV_SSE_2_MEM;
		...
		case IR_TYPE_DECL:
		{
			...
			else if (assign.lhs.type == IR_TYPE_DECL && (assign.rhs.type == IR_TYPE_DECL || assign.rhs.type == IR_TYPE_ON_STACK))
			{
				...
				if(assign.lhs.is_float)
					GenX64ToIrValDecl2(lang_stat, ret, &lhs, &assign.lhs, false, false, AllocFloatReg(lang_stat));
				else
					GenX64ToIrValDecl2(lang_stat, ret, &lhs, &assign.lhs, false, false, AllocReg(lang_stat));
				//assign.lhs.deref--;
				...
				*/
				...
				GenX64ToIrValDecl2(lang_stat, ret, &dst, &assign.to_assign, true, false);
			...
			else if (assign.lhs.type == IR_TYPE_DECL && assign.rhs.type == IR_TYPE_F32)
			{
				...
				GenX64AutomaticDeclDeref(lang_stat, ret, deref, &reg, &voffset, reg_sz, assign.lhs.is_float, assign.lhs.is_packed_float, 
					assign.lhs.decl->type.type == TYPE_VECTOR, false);
		...
		case IR_TYPE_REG:
		{
					 ...
					 (assign.lhs.type == IR_TYPE_ARG_REG && assign.rhs.type == IR_TYPE_REG || assign.rhs.type == IR_TYPE_RET_REG))
			{
				...
				GenX64ToIrValReg2(lang_stat, ret, &rhs, &assign.rhs, false, lhs.is_packed_float);
				...
				(assign.lhs.type == IR_TYPE_ON_STACK && assign.rhs.type == IR_TYPE_ON_STACK))
			{
				...
				//assign.rhs.deref--;
				if(assign.rhs.is_float)
					GenX64ToIrValDecl2(lang_stat, ret, &rhs, &assign.rhs, false, lhs.is_packed_float, AllocFloatReg(lang_stat) );
				else
					GenX64ToIrValDecl2(lang_stat, ret, &rhs, &assign.rhs, false, lhs.is_packed_float, AllocReg(lang_stat));
				...
				GenX64ToIrValReg2(lang_stat, ret, &dst, &assign.to_assign, true, assign.to_assign.is_packed_float);
				//GenX64ToIrValReg2(lang_stat, ret, &dst, &assign.to_assign, true, false);
		...
		*/
        ...
        //BREAK(cur_line == 259)
			...
			else if (ir->bin.lhs.type == IR_TYPE_DECL && ir->bin.rhs.type == IR_TYPE_F32)
			{
				...
				GenX64ToIrValDecl2(lang_stat, ret, &lhs, &ir->bin.lhs, false, false);
				//ir->bin.lhs.deref--;
				AllocSpecificReg(lang_stat, lhs.reg);
			...
			else if ((ir->bin.lhs.type == IR_TYPE_REG || ir->bin.lhs.type == IR_TYPE_RET_REG)&& ir->bin.rhs.type == IR_TYPE_DECL)
			{
				...
				GenX64ToIrValDecl2(lang_stat, ret, &rhs, &ir->bin.rhs, false, lhs.is_packed_float);
				AllocSpecificReg(lang_stat, rhs.reg);
			...
			else if (ir->bin.lhs.type == IR_TYPE_DECL && ir->bin.rhs.type == IR_TYPE_DECL)
			{
				...
				if (ir->bin.lhs.deref > -1)
					reg = AllocReg(lang_stat);
				GenX64ToIrValDecl2(lang_stat, ret, &lhs, &ir->bin.lhs, true, false, reg);
				//ir->bin.lhs.deref--;
				AllocSpecificReg(lang_stat, lhs.reg);
		...
		{
			/*
			if(cur_line== 1422 && gen_state->cur_func->name == "tests")
			{
				raise(SIGTRAP);
			}
			*/
		...
		case IR_CAST_INT_TO_F32:
		{
			...
			case IR_TYPE_DECL:
			{
				...
				GenX64ToIrValDecl2(lang_stat, ret, &rhs, &ir->bin.rhs, true, false);
