...
void ExitProcess(int val) { _exit(val); }
#else
#include <dsound.h>
...
void Draw(dbg_state *dbg) {
  ...
  // glDrawArrays(GL_TRIANGLES, 0, 3);
  //*(int*)&dbg->mem_buffer[RET_1_REG * 8] = glfwWindowShouldClose((GLFWwindow
  //*)(long long)wnd);
}
...
void ImGuiPopItemWidth(dbg_state *dbg) { ImGui::PopItemWidth(); }
...
void ImGuiSameLine(dbg_state *dbg) { ImGui::SameLine(); }
...
void ImGuiEnd(dbg_state *dbg) { ImGui::End(); }
...
void ImGuiEndChild(dbg_state *dbg) { ImGui::EndChild(); }
...
void ShouldClose(dbg_state *dbg) {
  ...
  *(int *)&dbg->mem_buffer[RET_1_REG * 8] =
...
int GenRawTexture(dbg_state *dbg) {
  ...
  GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sz_x, sz_y, 0, GL_RGBA,
...
void CreateMesh(dbg_state *dbg) {
  ...
  GL_CALL(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, vertex_size,
                                (void *)(3 * sizeof(float))));
...
void LoadModelBase(dbg_state *dbg, own_std::string &full_path) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, VBO);
  glBufferData(GL_ARRAY_BUFFER, mesh->mNumVertices * 3 * sizeof(float),
...
void Init3D(dbg_state *dbg) {
  ...
  perspective(gl_state->projection, 70.0f * (3.14159f / 180.0f),
              (float)gl_state->width / (float)gl_state->height, 0.1f, 500.0f);
...
void OpenWindow(dbg_state *dbg) {
  ...
  if (gl_state->glfw_window) {
    ...
    gl_state->width = wnd_width;
    gl_state->height = wnd_height;
    *(long long *)&dbg->mem_buffer[RET_1_REG * 8] =
...
void GetInstRealAddr(dbg_state *dbg) {
  ...
  *(u64 *)&dbg->mem_buffer[RET_1_REG * 8] =
