		...
		case AST_CALL:
		{
			...
			*/
			...
			if (there_is_exps_in_stack)
			{
				...
				ir.assign.to_assign.is_packed_float = e->call.fdecl->ret_type.type == TYPE_VECTOR;
				...
				ir.assign.lhs.is_packed_float = e->call.fdecl->ret_type.type == TYPE_VECTOR;
		...
		case AST_BINOP:
		{
			...
			*/
			...
			// reusing the reg
				...
				if(ir.assign.to_assign.type == IR_TYPE_REG && ir.assign.lhs.type == IR_TYPE_REG 
					&& ir.assign.rhs.type == IR_TYPE_REG && ir.assign.to_assign.reg == ir.assign.rhs.reg)
				{
...
void GetIRFromAst(lang_state *lang_stat, ast_rep *ast, own_std::vector<ir_rep> *out, ir_val *top)
{
    ...
    case AST_ON:
	{
		...
		FOR_VEC(cur_cond, ast->on.exprs)
		{
			...
			{
				if(cur_cond->cond->type == AST_BINOP && cur_cond->cond->op == T_COMMA)
				{
    ...
    case AST_RET:
	{
		...
		ir.ret.assign.to_assign.is_packed_float = lang_stat->cur_func->ret_type.type == TYPE_VECTOR;
        ...
        case T_EQUAL:
        {
			...
			if (ast->op == T_PLUS_EQUAL || ast->op == T_MINUS_EQUAL)
			{
				...
				if(ir.assign.to_assign.type == IR_TYPE_DECL && ir.assign.to_assign.deref >= 0)
				{
					...
					new_ir.assign.to_assign.is_packed_float = ir.assign.to_assign.is_packed_float;
