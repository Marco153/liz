...
{
  if(ir.type == IR_BIN && ir.bin.op == T_EQUAL && (int)ir.bin.rhs.type > 1000)
  {
    HERE()
  }
  ...
  case AST_CAST:
  {
    ...
    else
    {
      ...
      if(ret.ptr > 0)
      {
        ret.reg_sz = 8;
        ret.kind = IR_VAL_ADDR;
        LoadDerefs(lang_stat, &state->cur_block->irs, &ret, ret.deref - 1, ret.deref);
        ret.kind = IR_VAL_ADDR;
      }
    ...
    case T_PLUS:
    {
      ...
      for(int i= 1; i < ast->e_holder.expr.size(); i++)
      {
        ...
        if(rhs.kind != IR_VAL_VALUE)
        {
          ...
          if(rhs.is_float)
          {
            FreeReg(lang_stat, rhs.reg, false);
          }
          //rhs.type = IR_TYPE_REG_MEM;
        }
