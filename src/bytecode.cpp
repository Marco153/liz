...
void CreateRegToReg(byte_code *bc, char byte_op, char word_op, machine_code *ret, bool rhs_is_final_reg = false)
{
	char reg_dst = FromBCRegToAsmReg(bc->bin.lhs.reg);
	char reg_src = bc->bin.rhs.reg;
	if(!rhs_is_final_reg)
		reg_src = FromBCRegToAsmReg(bc->bin.rhs.reg);
...
void CreateImmToReg(char r0_byte_imm_byte, char r0_imm, char inst_reg, byte_code *bc, machine_code &ret)
{
	...
	{
    char is_rex = IS_FLAG_ON(bc->bin.lhs.reg, 0x80);
    *(char *)&is_rex *= 2;
		...
		if(bc->bin.rhs.u64 >= 0x80)
		{
			...
			if(bc->bin.lhs.reg == 0)
			{
				AddPreMemInsts(bc->bin.lhs.reg_sz, r0_byte_imm_byte, r0_imm, is_rex, ret.code, bc->bin.lhs.reg == 4);
				AddImm(bc->bin.rhs.u64, bc->bin.lhs.reg_sz, ret);

			}
			else
			{
				AddPreMemInsts(bc->bin.lhs.reg_sz, 0x80, 0x81, is_rex, ret.code, bc->bin.lhs.reg == 4);
...
{
#define CHANGE_JMP_DST_BC ret.jmp_rels.back().dst_bc = &bcodes[i + bc->val];
	int i = 0;
  ...
  own_std::vector<ir_rep>* func_irs;
		...
		case SYSCALL:
    {
      ...
      */
      ...
      //HERE()
			ret.code.emplace_back(0x66);
			ret.code.emplace_back(0x0f);
			ret.code.emplace_back(0x7e);
			ret.code.emplace_back(0xc0 | (bc->bin.rhs.reg << 3) | bc->bin.lhs.reg);
		}break;
		case MOV_R_2_SSE:
		{
      ...
      ret.code.emplace_back(3 << 6 | (src&0x7) | (dst&0x7)<<3);
		...
		case MOV_M_2_REG_PARAM:
		{
			...
			if (bc->bin.lhs.reg < MAX_CALL_REGS)
			{
				...
				{
					CreateMemToReg(&*bc, 0x8a, 0x8b, true, ret, 5, true);
				}
				else
				{
					CreateMemToReg(&*bc, 0x8a, 0x8b, false, ret, 5, true);
				}
		...
		case SET_SYSCALL:
    {
      is_syscall = true;
		...
		case MOV_REG_PARAM_2_REG:
		{
			if ((bc->bin.rhs.reg & 0xf) < MAX_CALL_REGS)
			{
				...
				else
				{
					...
					CreateRegToReg(&*bc, 0x88, 0x89, &ret, true);
					bc->bin = bin;
				}
		...
		case STORE_REG_PARAM:
		{
			if ((bc->bin.rhs.reg & 0xf) < MAX_CALL_REGS)
			{
				...
				{
					//char stack_reg = (MAX_CALL_REGS - prev_reg - ) * 8 + 32;
					//raise(SIGTRAP);
					auto bin = bc->bin;
					bc->bin.rhs.reg = final_reg;
					//bc->bin.rhs.voffset  = stack_reg;
      ...
      AddSSEExtendedByte(&dst, &zero, &ret);

			ret.code.emplace_back(0x0f);
			ret.code.emplace_back(0x2f);

      int offset = bc->bin.rhs.voffset;
			AddModRM(true, offset, src, dst & 0xf, ret);
			if (offset != 0)
      ...
      AddSSEExtendedByte(&dst, &src, &ret);
			...
			char mod = MakeModRM(false, 0, src, dst);
      ...
      ret.code.emplace_back(bc->bin.rhs.i);
		...
		case INST_LEA:
		{
			...
			char is_rex = IS_FLAG_ON(base_reg, 0x80);
			*(char*)&is_rex *= 2;
			*(char*)&is_rex |= IS_FLAG_ON(dst_reg, 0x80);
		...
		case REP_MOVSB:
    {
		...
		case MUL_I_2_R:
		{
			char base_reg = FromBCRegToAsmReg(bc->bin.lhs.reg);
      char is_rex = IS_FLAG_ON(base_reg, 0x80);
      is_rex = is_rex | (is_rex << 1);

			base_reg &= 0xf;
			bool less_than_byte = bc->bin.rhs.u32 < 0x80;

			if(less_than_byte)
				AddPreMemInsts(bc->bin.lhs.reg_sz, 0x6b, 0x6b, is_rex, ret.code);
			else
				AddPreMemInsts(bc->bin.lhs.reg_sz, 0x69, 0x69, is_rex, ret.code);
		...
		case XOR_M_2_R:
			CreateMemToReg(&*bc, 0x30, 0x31, false, ret);
			break;
		case XOR_I_2_M:
