...
mem_chunk *GetUnallocatedChunk(mem_alloc *alloc) {
  ...
  if (!ret) {
    for (int i = 0; alloc->chunks_cap; i += 4) {
      ...
      _mm_prefetch((char *)&alloc->all[(i + 4) % alloc->chunks_cap],
                   _MM_HINT_T0);

      if (IS_FLAG_OFF(cur0->flags, CHUNK_ALLOCATED)) {
        ret = cur0;
        heap_insert_unallocated(alloc,
                                &alloc->all[(i + 1) % alloc->chunks_cap]);
        break;
      }
      if (IS_FLAG_OFF(cur1->flags, CHUNK_ALLOCATED)) {
        ret = cur1;
        heap_insert_unallocated(alloc,
                                &alloc->all[(i + 2) % alloc->chunks_cap]);
        break;
      }
      if (IS_FLAG_OFF(cur2->flags, CHUNK_ALLOCATED)) {
        ret = cur2;
        heap_insert_unallocated(alloc,
                                &alloc->all[(i + 3) % alloc->chunks_cap]);
        break;
      }
      if (IS_FLAG_OFF(cur3->flags, CHUNK_ALLOCATED)) {
        ret = cur3;
        heap_insert_unallocated(alloc,
                                &alloc->all[(i + 4) % alloc->chunks_cap]);
        break;
      }
      /*
      mem_chunk* cur = &alloc->all[i];
      _mm_prefetch((char*)&alloc->all[(i + 1) % alloc->chunks_cap],
      _MM_HINT_T0);

      if (IS_FLAG_OFF(cur->flags, CHUNK_ALLOCATED))
      {
              ret = cur;
              alloc->probable_unallocated = &alloc->all[(i + 1) %
      alloc->chunks_cap]; break;
      }
      */
    }
  }
...
void maybe_join_to_right(mem_chunk *prev, mem_chunk *cur, mem_chunk *next,
                         heap_free_ret_info *hinfo) {
  bool contiguous_with_cur_next =
      (cur->addr + cur->size * BYTES_PER_CHUNK) == next->addr;
...
void heap_insert_unallocated(mem_alloc *alloc, mem_chunk *a) {
  if (a < alloc->all || a > (alloc->all + alloc->chunks_cap) ||
      IS_FLAG_ON(a->flags, CHUNK_ALLOCATED))
    return;
...
void heap_free(mem_alloc *alloc,
               char *ptr) //, heap_free_ret_info *ret = nullptr)
{
  ...
  if (chunk_to_free->addr > cur->addr) {
    ...
    // TODO: create a var put these two vals in each bit, and create a swich
    // statetement so that we
    //  wont need if and elses
    bool contiguous_with_cur =
        (cur->addr + cur->size * BYTES_PER_CHUNK) == chunk_to_free->addr;
    bool contiguous_with_cur_next =
        (chunk_to_free->addr + chunk_to_free->size * BYTES_PER_CHUNK) ==
        cur->next->addr;
...
char *linear_alloc_func(linear_alloc *alloc,
                        int size) //, mem_chunk **out = nullptr)
{
  auto ret = alloc->data + alloc->cur;
  alloc->cur += size;
  ASSERT(alloc->cur < alloc->max);
  return ret;
}
void linear_free_stub(linear_alloc *alloc,
                      void *addr) //, mem_chunk **out = nullptr)
{}
...
void InitMemAlloc(mem_alloc *alloc) {
  ...
  long long total_size =
      mem_chunks_size + raw_mem_size + in_use_hash_sz + unallocated_sz;
  ...
  char *buffer = start;
  printf("mem chunks %dmb\n", mem_chunks_size / 1024 / 1024);
  printf("raw mem %dmb\n", raw_mem_size / 1024 / 1024);
  printf("hash table size %dmb\n", in_use_hash_sz / 1024 / 1024);
