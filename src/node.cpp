...
#define CMP_NTYPE_BIN(a, ttype)                                                \
  ((a)->type == node_type::N_BINOP && (a)->t->type == ttype)

void AddStructMembersToScopeWithUsing(lang_state *, type_struct2 *decl,
                                      scope *scp, node *by_name_nd);
...
void ModifyFuncDeclToName(lang_state *lang_stat, func_decl *fdecl, node *n,
                          scope *scp);
own_std::string StringifyNode(node *n);
bool CallNode(lang_state *, node *ncall, scope *scp, type2 *ret_type,
              decl2 * = nullptr);
unit_file *AddNewFile(lang_state *, own_std::string name, own_std::string path);
...
own_std::string FuncNameWithTempls(lang_state *, own_std::string fname,
                                   own_std::vector<type2> *types);
node *ParseString(lang_state *, own_std::string str, int = 0);
decl2 *DeclareDeclToScopeAndMaybeToFunc(lang_state *, own_std::string name,
                                        type2 *tp, scope *scp,
                                        node *nd = nullptr);
...
decl2 *FindIdentifier(own_std::string &name, scope *scp, type2 *ret_type,
                      int = 0);
...
node *MakeFuncCallArgs(lang_state *, own_std::string op_func, node *ref,
                       own_std::vector<node *> &args, token2 *);
...
bool FunctionIsDone(lang_state *, node *n, scope *scp, type2 *ret_type,
                    int flags);
...
void DescendComma(lang_state *, node *n, scope *scp,
                  own_std::vector<comma_ret> &ret);
...
#define PSR_FLAGS_RETURN_CALL_ARGS_FROM_SCP 0x2000000
...
own_std::string scope::Print(int indent) {
  ...
  FOR_VEC(decl, vars) {
    ...
    snprintf(buffer, 512, "%sname: %s, type: %s, offset: %d\n", tabs,
             d->name.c_str(), TypeToString(d->type).c_str(), d->offset);
    ret += buffer;
  }
...
template <class T> struct tag_strct {
  long long tag;
  T val;
};
...
char *GetFuncBasedOnAddr(lang_state *lang_stat, unsigned long long offset) {
  FOR_VEC(it, lang_stat->global_funcs) {
    ...
    auto start_addr =
        (unsigned long long)lang_stat->GetCodeAddr(f->code_start_idx);
    auto end = (unsigned long long)lang_stat->GetCodeAddr(f->code_start_idx +
                                                          f->code_size);
...
void NewTypeDataSymbol(lang_state *lang_stat, int offset, char *name) {
  // lang_stat->symbols_offset_on_type_sect[own_std::string(name)] = offset;
}
...
bool node_iter::is_operator(token2 *tkn, int *precedence) {
  ...
  if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_LAMBDA_ARGS) &&
      tkn->type == tkn_type2::T_PIPE) {
  ...
  if (tkn->type == tkn_type2::T_EQUAL &&
      (tkn + 1)->type == tkn_type2::T_EQUAL) {
    get_tkn();
    *precedence = 1;
    return true;
  }
...
void node_iter::SetNodeScopeIdx(lang_state *lang_stat, node **nd,
                                unsigned char val, int scope_start,
                                int scope_end) {
...
bool CompareTypes(type2 *lhs, type2 *rhs, bool assert = false) {
  if (rhs->type == enum_type2::TYPE_STR_LIT &&
      lhs->type == enum_type2::TYPE_U8 && lhs->ptr == 1)
    return true;
  bool cond = false;

  if (rhs->type == enum_type2::TYPE_STATIC_ARRAY &&
      (rhs->tp->type == lhs->type ||
       rhs->tp->type == TYPE_CHAR && lhs->type == TYPE_U8 ||
       lhs->type == TYPE_VOID) &&
  ...
  case enum_type2::TYPE_STATIC_ARRAY: {
    ...
    cond = (are_arrays_same_type &&
            (rhs->ar_size == lhs->ar_size || rhs->ar_size == -1)) ||
  ...
  case enum_type2::TYPE_STR_LIT: {
    cond = rhs->type == enum_type2::TYPE_STR_LIT ||
           (rhs->type == enum_type2::TYPE_U8 && rhs->ptr == 1);
    if (assert && !cond)
      ASSERT(false)
  } break;
  case enum_type2::TYPE_ENUM: {
    ...
    if (lhs->e_decl->name == "type_enum") {
      ...
      if (assert && !cond)
        ASSERT(false)
    } else if (rhs->type == TYPE_STRUCT && lhs->from_enum->type.strct &&
               rhs->strct) {
      cond = true;
    }
    /*
    else if (lhs->e_decl->type == TYPE_STRUCT_TYPE &&
    IS_FLAG_ON(lhs->e_decl->type.strct->flags, TP_STRCT_ETRUCT))
    {
            cond = rhs->from_enum
    }
    */
  ...
  case enum_type2::TYPE_INT:
    cond =
        (rhs->type == enum_type2::TYPE_S64 ||
         ...
         rhs->type == enum_type2::TYPE_U8 || rhs->type == enum_type2::TYPE_INT);
    break;
  ...
  case enum_type2::TYPE_S32:
    cond = (rhs->type == enum_type2::TYPE_S32 ||
            rhs->type == enum_type2::TYPE_S16 ||
            rhs->type == enum_type2::TYPE_S8 ||
            (rhs->type == enum_type2::TYPE_INT && rhs->s32 >= -2147483646 &&
             rhs->s32 <= 2147483647));
    break;
  case enum_type2::TYPE_S16:
    cond = (rhs->type == enum_type2::TYPE_S16 ||
            rhs->type == enum_type2::TYPE_S8 ||
            (rhs->type == enum_type2::TYPE_INT && rhs->s64 >= -32768 &&
             rhs->s64 <= 32767));
    break;
  ...
  case enum_type2::TYPE_U64:
    cond = rhs->type == enum_type2::TYPE_U64 ||
           rhs->type == enum_type2::TYPE_U32 ||
           rhs->type == enum_type2::TYPE_U16 ||
           rhs->type == enum_type2::TYPE_U8 || rhs->ptr > 0 ||
  ...
  case enum_type2::TYPE_U16:
    cond = (rhs->type == enum_type2::TYPE_U16 ||
            rhs->type == enum_type2::TYPE_U8 ||
            (rhs->type == enum_type2::TYPE_INT && rhs->i >= 0 &&
             rhs->u32 <= 0xffff));
    break;
  case enum_type2::TYPE_U8:
    cond = (rhs->type == enum_type2::TYPE_U8 ||
            rhs->type == enum_type2::TYPE_CHAR ||
            (rhs->type == enum_type2::TYPE_INT && rhs->i >= 0 &&
             rhs->u16 <= 0xff));
    break;
  case enum_type2::TYPE_VOID:
    cond = lhs->ptr > 0 && rhs->ptr > 0 || rhs->type == lhs->type ||
           lhs->ptr > 0 && rhs->type == TYPE_STR_LIT;
  ...
  default: {
    ...
    if (assert) {
      ASSERT(lhs->type == rhs->type)
               ...
               !(lhs->type == TYPE_STRUCT && rhs->type == TYPE_STRUCT_TYPE))
      return false;

    if (lhs->type == enum_type2::TYPE_STRUCT) {
      cond = (lhs->strct->name == rhs->strct->name);
    } else if (lhs->type == enum_type2::TYPE_ARRAY ||
               lhs->type == enum_type2::TYPE_ARRAY) {
...
void node_iter::CreateCondAndScope(node **n) {
  ...
  if (peek_tkn()->type != tkn_type2::T_OPEN_CURLY) {
    ...
    SetNodeScopeIdx(lang_stat, &(*n)->r, cur_scope_count++, scope_start,
                    scope_end);
...
node *node_iter::parse_func_like() {
  ...
  auto peek = peek_tkn();
  if (!(peek->type == T_SEMI_COLON || peek->type == T_NEW_LINE ||
        peek->type == T_OPEN_CURLY || peek->type == T_CLOSE_PARENTHESES ||
        peek->type == T_COMMA)) {
    ReportMessage(lang_stat, peek, "unexpected token after declaring args");
    ExitProcess(1);
  }
  ...
  if (t_tp == tkn_type2::T_OPEN_CURLY) {
    ...
    n->flags |= NODE_FLAGS_IS_SCOPE;
    SetNodeScopeIdx(lang_stat, &n->r, cur_scope_count++, n->t->line,
                    n->t->line);
...
node *node_iter::parse_str(own_std::string &str, int *i, int start_line) {
  int end = *i + 1;
  ...
  *i = end;
  ...
  auto new_tkns = (own_std::vector<token2> *)__lang_globals.alloc(
      __lang_globals.data, sz_tkns);
  memset(new_tkns, 0, sz_tkns);

  char *aux = (char *)std_str_to_heap(lang_stat, &new_str);
  Tokenize2(aux, new_str.size(), new_tkns);
  FOR_VEC(t, *new_tkns) { t->line += start_line - 1; }
  node_iter niter(new_tkns, lang_stat);
  return niter.parse_(0, parser_cond::LESSER_EQUAL);
}
node *node_iter::parse_until_head_is_word() {
  lang_stat->flags |= PSR_FLAGS_BREAK_WHEN_NODE_HEAD_IS_WORD;
  EatNewLine();
  node *ret = parse_(0, parser_cond::LESSER_EQUAL);
  lang_stat->flags &= ~PSR_FLAGS_BREAK_WHEN_NODE_HEAD_IS_WORD;
  EatNewLine();
  return ret;
}
...
node *node_iter::parse_expr() {
  ...
  case tkn_type2::T_HASHTAG: {
    ...
    if (peek->str == "do") {
      ...
      get_tkn();
    } else if (peek->str == "assign_ser_func") {
      get_tkn();
      EatNewLine();
      n->type = N_ASSIGN_SER_FUNC;
      n->r = parse_expr();
      n->r->flags &= ~NODE_FLAGS_COMMA_INSIDE_PARENTHESES;
      lang_stat->flags |= PSR_FLAGS_IMPLICIT_SEMI_COLON;
    ...
    while (i < str.size()) {
      ...
      if (str[i] == '{') {
        if (str[clamped] == '\\') {
          ...
          open_curly_open = 1;
          args.emplace_back(parse_str(str, &i, cur_tkn->line));
          if (str[i] != '}') {
            ReportError(lang_stat, cur_tkn->line, cur_tkn->line_offset,
                        "string interpolation error: expected closing '}'", 0);
            ExitProcess(1);
          }
          args.back()->t->line = cur_tkn->line;

          ref_str += '%';
        }
      } else if (str[i] == '}' && str[clamped] == '\\') {
        str.erase(clamped, 1);
        ref_str.pop_back();
        ref_str += '}';
        continue;
  ...
  case tkn_type2::T_WORD: {
    ...
    if (GetTypeFromTkns(cur_tkn, n->decl_type)) {
      ...
      if (peek_tkn()->str == "as") {
        ...
        if (n->r->type == N_STRUCT_DECL) {
          ...
          node *decl_nd = NewBinOpNode(lang_stat, name_nd, tkn_type2::T_COLON,
      ...
      cur_tkn = peek_tkn();
      if (cur_tkn->type != tkn_type2::T_SEMI_COLON &&
          cur_tkn->type != tkn_type2::T_NEW_LINE)
        n->r = parse_(PREC_SEMI_COLON, parser_cond::LESSER_EQUAL);
    ...
    } else if (cur_tkn->str == "on") {
      n->type = N_ON;
      auto on = (on_expr *)AllocMiscData(lang_stat, sizeof(on_expr));
      n->on = on;

      on->main = parse_until_head_is_word();
      ...
      while (cur_tkn->type != T_CLOSE_CURLY) {
        ...
        if (cur_cond.cond->type == N_IDENTIFIER &&
          ...
          on->exprs.emplace_back(cur_cond);
        }
        EatNewLine();
        cur_tkn = peek_tkn();
      }
      ...
      */
      ...
      while (IsTknWordStr(peek_tkn(), "else")) {
        ...
        if (IsTknWordStr(peek_tkn(), "else") &&
          ...
          if (peek_tkn()->type != tkn_type2::T_OPEN_CURLY) {
            lang_stat->flags |= PSR_FLAGS_BREAK_WHEN_NODE_HEAD_IS_WORD |
                                PSR_FLAGS_SCOPE_WHITHOUT_CURLY;
            ...
            SetNodeScopeIdx(lang_stat, &cur_node->r, cur_scope_count++,
        ...
        // else if
        else if (IsTknWordStr(peek_tkn(), "else") &&
                 IsTknWordStr(peek_tkn() + 1, "if")) {
          ASSERT(IsTknWordStr(get_tkn(), "else"));
          get_tkn();
          CreateCondAndScope(&cur_node->l);
  ...
  case tkn_type2::T_OPEN_PARENTHESES: {
    ...
    if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_ON_FUNC_CALL) &&
        IS_FLAG_OFF(lang_stat->flags, PSR_FLAGS_BREAK_WHEN_NODE_HEAD_IS_WORD) &&
        peek_tkn()->type == T_OPEN_CURLY) {
      ReportMessage(lang_stat, cur_tkn,
                    "It seems like you're trying to declare a function but you "
                    "forgot to type this part '::fn(..)");
    }
  ...
  if (peek_tkn()->type == tkn_type2::T_OPEN_CURLY) {
    auto peek = peek_tkn();
    if (peek->type == T_OPEN_CURLY &&
        IS_FLAG_OFF(lang_stat->flags,
                    PSR_FLAGS_ON_STRUCT_DECL | PSR_FLAGS_ON_FUNC_DECL |
                        PSR_FLAGS_BREAK_WHEN_NODE_HEAD_IS_WORD |
                        PSR_FLAGS_IMPLICIT_SEMI_COLON)) {
      int a = 0;
...
#define PARSER_CHECK                                                           \
  if (is_operator(peek_tkn(), &cur_prec) ||                                    \
      peek_tkn()->type == T_CLOSE_BRACKETS) {                                  \
    bool final_cond;                                                           \
    switch (pcond) {                                                           \
    case parser_cond::GREATER_EQUAL: {                                         \
      final_cond = cur_prec >= prec;                                           \
    } break;                                                                   \
    case parser_cond::LESSER_EQUAL: {                                          \
      final_cond = cur_prec <= prec;                                           \
    } break;                                                                   \
    case parser_cond::GREATER: {                                               \
      final_cond = cur_prec > prec;                                            \
    } break;                                                                   \
    case parser_cond::LESSER: {                                                \
      final_cond = cur_prec < prec;                                            \
    } break;                                                                   \
    case parser_cond::EQUAL: {                                                 \
      final_cond = cur_prec == prec;                                           \
    } break;                                                                   \
    case parser_cond::NEQUAL: {                                                \
      final_cond = cur_prec != prec;                                           \
    } break;                                                                   \
    }                                                                          \
    if (final_cond || (peek_tkn()->type == tkn_type2::T_EOF) ||                \
        peek_tkn()->type == T_CLOSE_BRACKETS) {                                \
...
void node_iter::CheckTwoBinaryOparatorsTogether(node *cur_node) {
  ...
  bool two_binary_operators_together =
      (is_operator(peek_tkn(), &dummy_prec) &&
       is_operator(cur_node->t, &dummy_prec) && !is_unary) &&
      tkn->type != tkn_type2::T_CLOSE_PARENTHESES &&
      tkn->type != tkn_type2::T_CLOSE_CURLY && tkn->type != tkn_type2::T_EOF &&
      tkn->type != tkn_type2::T_OPEN_CURLY;
  if (two_binary_operators_together)
    ReportMessageOne(lang_stat, tkn, "unexpected token '%s'",
                     (char *)tkn->ToString().c_str());
}
...
node *node_iter::parse_(int prec, parser_cond pcond) {
  ...
  bool is_file =
      lang_stat->cur_file && lang_stat->cur_file->name == "entity.lng";

  auto begin_tkn = peek_tkn();
  if (begin_tkn->type == T_CLOSE_PARENTHESES ||
      begin_tkn->type == T_CLOSE_CURLY) {
  ...
  while (1) {
    ...
    if (break_when_node_head && cur->type == T_NEW_LINE &&
        (cur + 1)->type != T_WORD) {
      EatNewLine();
    }

    return_without_semicolon_on_scope_end =
        cur_is_ident_or_int && (next_is_else || next_is_curly);
    ...
    if ((cur_is_ident_or_int && break_when_node_head) ||
        return_without_semicolon_on_scope_end) {
      return cur_node->l;
    }
    if (rev && cur->type == T_WORD &&
        (cur->str == "return" || cur->str == "if" || cur->str == "else")) {
      return cur_node->l;
    }
    ...
    case tkn_type2::T_OPEN_PARENTHESES: {
      ...
      if (peek_tkn()->type == tkn_type2::T_OPEN_CURLY &&
          IS_FLAG_OFF(lang_stat->flags,
                      PSR_FLAGS_BREAK_WHEN_NODE_HEAD_IS_WORD)) {
        ReportMessageOne(
            lang_stat, cur_node->t,
            "Unexpected token. Maybe you are trying to declare a function",
            nullptr);
        ExitProcess(1);
      }
    ...
    tkn = peek_tkn();
    if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_SCOPE_WHITHOUT_CURLY) &&
        (tkn->type == T_SEMI_COLON || tkn->type == T_NEW_LINE))
      return cur_node->l;
    ...
    if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_IMPLICIT_SEMI_COLON)) {
      ...
      if (!IsTknWordStr(tkn, "as")) {
        bool val =
            tkn->type != tkn_type2::T_WORD && tkn->type != tkn_type2::T_FLOAT &&
            tkn->type != tkn_type2::T_STR_LIT &&
            tkn->type != tkn_type2::T_STR_LIT && tkn->type != tkn_type2::T_INT;
    ...
    if (cur_prec == PREC_SEMI_COLON) {
      ...
      if (IS_PRS_FLAG_ON(PSR_FLAGS_ON_FUNC_CALL) &&
          !IS_PRS_FLAG_ON(PSR_FLAGS_ON_ETRUCT)) {
      ...
      cur_node->type = node_type::N_BINOP;
      begin_tkn = peek_tkn();
      if (begin_tkn->type == T_CLOSE_CURLY &&
          is_operator(cur_node->l->t, &dummy_prec) &&
          IS_FLAG_OFF(lang_stat->flags, PSR_FLAGS_ON_STRUCT_DECL)) {
        ReportMessageOne(lang_stat, cur_node->l->t, "unexpected token '%s'",
                         (char *)begin_tkn->ToString().c_str());
      }
    }
    ...
    if (tt->type == T_OPEN_CURLY && cur_node->type == N_BINOP &&
        cur_node->t->type == T_POINT) {
      cur_node->r = parse_expr();
      ...
      */
      if (peek_tkn()->type != tkn_type2::T_MUL &&
          peek_tkn()->type != tkn_type2::T_MINUS) {
        PARSER_CHECK
      }
      if (cur->type == T_CLOSE_PARENTHESES) {
        REPORT_ERROR(
            cur->line, cur->line_offset,
            VAR_ARGS("unexpected token '%s'\n", cur->ToString().c_str()))
        ExitProcess(1);
      }
...
int CheckForHashtags(lang_state *lang_stat, node *n, func_decl *fdecl,
                     scope *scp);
bool CheckFuncRetType(lang_state *lang_stat, func_decl *fdecl, scope *scp) {
  char msg_hdr[256];
  auto fnode = fdecl->func_node;
  auto has_hash_tags =
      CheckForHashtags(lang_stat, fdecl->func_node, fdecl, scp);
  if (has_hash_tags == 1) {
    return false;
  }
  // return type
  if (fnode->l->r != nullptr) {
    ...
    if (fdecl->ret_type.type == TYPE_ENUM &&
        fdecl->ret_type.e_decl->type.type == TYPE_ENUM_TYPE) {
  ...
  // asserting we're returning somethins at the end of the scope, but excluding
  // function that have templates
  // getting the correct last stmnt
  if (fdecl->ret_type.type != enum_type2::TYPE_VOID && fnode->r->r->r &&
      IS_FLAG_OFF(fdecl->flags, FUNC_DECL_TEMPLATED)) {
    ...
    if (can_report) {
      REPORT_ERROR(
          fdecl->func_node->t->line, fdecl->func_node->t->line_offset,
          VAR_ARGS("function %s must return a value\n", fdecl->name.c_str()))
      ExitProcess(1);
    }
  }
  return true;
...
node *ParseString(lang_state *lang_stat, own_std::string str, int start_line) {
  auto tkns = (own_std::vector<token2> *)AllocMiscData(
      lang_stat, sizeof(own_std::vector<token2>));
  ...
  if (start_line != 0) {
    FOR_VEC(t, *tkns) { t->line = start_line; }
  }
...
#define IS_COMMA(n)                                                            \
  ((n)->type == node_type::N_BINOP && (n)->t->type == tkn_type2::T_COMMA)
#define IS_DECL(n) ((n)->type == node_type::N_DECL)
#define IS_COLON(n)                                                            \
  ((n)->type == node_type::N_BINOP && (n)->t->type == tkn_type2::T_COLON)
#define IS_EQUAL(n)                                                            \
  ((n)->type == node_type::N_BINOP && (n)->t->type == tkn_type2::T_EQUAL)
...
void DescendComma(lang_state *lang_stat, node *n, scope *scp,
                  own_std::vector<comma_ret> &ret) {
  if (IS_COMMA(n) &&
      IS_FLAG_OFF(n->flags, NODE_FLAGS_COMMA_INSIDE_PARENTHESES)) {
    ...
    if (IS_COLON(n)) {
      ...
      cret.decl.name = n->t->str;

      ret.emplace_back(cret);

    }
    // this is an ugly hack, an empty scope has an open curly child node, so in
    // the case of a declaration of enum, if is scope is empty, the DescedComma
    // will return an array with the open curly, which will cause a problem in
    // the parsing of the values of the enum
...
void DescendTemplates(node *n, scope *scp,
                      own_std::vector<template_expr> *ret) {
...
scope *NewScope(lang_state *lang_stat, scope *scp) {
  ...
  ret->parent = scp;
  // new(&ret->vars_map)own_std::hash_map<node *, func_decl *>(64);
  if (scp) {
...
bool TemplTypeFromStruct(type2 *in, type2 *out, type2 *arg,
...
// $TemplatesType
own_std::vector<type2> TemplatesTypeToLangArray(
    lang_state *lang_stat, own_std::vector<template_expr> *templates,
    own_std::vector<decl2 *> *original, own_std::vector<comma_ret> *args) {
  ...
  FOR_VEC(t, *templates) {
    ...
    if (given_arg->decl.type.type == TYPE_STATIC_ARRAY)
      ReportMessage(lang_stat, given_arg->n->t,
                    "Cannot create template from static array types. Try "
                    "dereferencing it.");

    // if the og func arg is a plain template, which is not inside another type
    if (og_a->type.type == enum_type2::TYPE_TEMPLATE &&
        og_a->type.templ_name == t->name) {
    ...
    {
      type2 tp;
      if (TemplTypeFromStruct(&og_a->type, &tp, &given_arg->decl.type,
                              t->name)) {
        ret.emplace_back(tp);
      }
    }
...
// $TemplatesType
own_std::string TemplatesTypeToString(own_std::vector<template_expr> *templates,
                                      func_decl *original,
...
own_std::vector<decl2 *> GetTemplateTypes(
    lang_state *lang_stat, own_std::vector<template_expr> *templates,
    own_std::vector<comma_ret> *args, scope *scp, func_decl *fdecl) {
      ...
      *(cur_t->final_type) = a->decl.type;
      ...
      if (fdecl != nullptr) {
        ...
        FOR_VEC(fparam, fdecl->args) {
          (*fparam)->AssignTemplate(lang_stat, cur_t->name, cur_t->final_type,
                                    &(*args)[cur_arg]);
...
own_std::vector<decl2 *>
DescendTemplatesToDecl(lang_state *lang_stat, node *n, scope *scp,
...
decl2 *FindIdentifier(own_std::string &name, scope *scp, type2 *ret_type,
  ...
  while (cur_scope != nullptr) {
    // self ref
    if (IS_FLAG_ON(cur_scope->flags, SCOPE_INSIDE_STRUCT) &&
        cur_scope->tstrct->name == name) {
...
#endif
  if (IS_FLAG_ON(scp->flags, SCOPE_RETURN_CALL_ARGS_FROM_SCP) &&
      scp->call_args.size() > 0) {
    auto last = scp->call_args.back();
    FOR_VEC(d, last) {
      if ((*d)->name == name) {
        return *d;
      }
  ...
  if (IS_FLAG_ON(scp->flags, SCOPE_INSIDE_STRUCT) &&
      scp->tstrct->name == name) {
    ret_type->flags |= TYPE_SELF_REF;
    return decl;
  }

  bool is_recursive_call =
      IS_FLAG_ON(scp->flags, SCOPE_INSIDE_FUNCTION) && scp->fdecl->name == name;
...
void DescendStmnt(lang_state *lang_stat, node *n, scope *scp) {
  ...
  while (true) {
    ...
    if (cur_node->l != nullptr &&
        IS_FLAG_OFF(cur_node->l->flags, NODE_FLAGS_IS_PROCESSED3)) {
    ...
    if (IS_FLAG_ON(scp->flags, SCOPE_INSIDE_FUNCTION)) {
      scp->fdecl->per_stmnt_strct_val_sz =
          max2(scp->fdecl->per_stmnt_strct_val_sz,
               lang_stat->cur_per_stmnt_strct_val_sz);
    }
    ...
    if (cur_node->r != nullptr &&
        IS_FLAG_OFF(cur_node->r->flags, NODE_FLAGS_IS_PROCESSED3)) {
...
int SetVariablesAddress(own_std::vector<decl2 *> *ar, int start,
                        int *in_out_biggest_type, int start_addr = 0) {
  int addr = start_addr;
  for (auto var = ar->begin() + start; var < ar->end(); var++) {
    auto t = *var;
    if (IS_FLAG_ON(t->flags, DECL_FROM_USING) ||
        t->type.type == TYPE_STRUCT_TYPE || t->type.is_const)
      continue;
    ...
    */
    ...
    //*in_out_biggest_type = other_biggest_type > cur_size ? other_biggest_type
    //: cur_size;
...
template <typename T> T GetExpressionValT(tkn_type2 tp, T a, T b) {
  switch (tp) {
  case tkn_type2::T_EQUAL:
    return b;
  case tkn_type2::T_LESSER_THAN:
...
bool TryInstantiateStruct(lang_state *lang_stat, type_struct2 *original,
                          own_std::string templ_name, scope *scp,
                          type_struct2 **ret, own_std::vector<comma_ret> &args,
                          type2 *from_type = nullptr) {
  auto strct = original;
  own_std::vector<template_expr> in_vec;

  type2 ret_type;
  memset(&ret_type, 0, sizeof(ret_type));
  auto instantions_exist = FindIdentifier(
      templ_name, scp, &ret_type, FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
  ...
  if (!instantions_exist) {
    auto tdecls = DescendTemplatesToDecl(lang_stat, strct->strct_node->l,
                                         strct->scp, &in_vec);
    ...
    // later right below, we dont wanna add the templates as the struct's
    // variables, so we're keeping track
    ...
    new_strct->name = own_std::string(new_strct->this_decl->name);
  } else {
  ...
  *ret = new_strct;
  ...
  new_strct->name = templ_name;

  new_strct->vars.assign(new_scope->vars.begin() + templates_idx_end,
                         new_scope->vars.end());
  int i = 0;
  FOR_VEC(cur_ptr, original->scp->vars) {
    ...
    if (IS_FLAG_ON(cur->flags, DECL_PTR_HAS_LEN)) {
      ...
      dst->flags = cur->flags;
      dst->len_for_ptr =
          FindIdentifier(cur->len_for_ptr->name, new_strct->scp, &dummy_tp);
    }
    i++;
  }
...
bool InstantiateArFromType(lang_state *lang_stat, type2 &ar_type, scope *scp,
                           type2 *ret_type, type_struct2 **ret_struct,
                           node *n) {
  type2 dummy_tp;
  ...
  if (!ar_strct) {
    ReportMessage(lang_stat, n->t,
                  "'array' not found. Try imporintg std.lng to your scope");
    ExitProcess(1);
  }
...
bool FromStaticArToAr(lang_state *lang_stat, type2 *ret_type, scope *scp,
                      node *n) {
  if (n->ar_lit_tp) {
  ...
  if (!InstantiateArFromType(lang_stat, *ret_type->tp, scp, ret_type,
                             &ret_struct, n)) {
...
node *GetLastStmntType(lang_state *lang_stat, node *n, scope *scp,
                       type2 &ret_type) {
...
bool NameFindingGetType(lang_state *lang_stat, node *n, scope *scp,
      ...
      if (n->ar_lit_tp) {
        ...
        if (top->l && !top->ar_lit_tp) {
          ...
          *ar_type_ptr->tp = ar_type;
          ...
          if (!InstantiateArFromType(lang_stat, ar_type, scp, &ret_type,
                                     &ret_struct, n))
            return false;
    ...
    case tkn_type2::T_AMPERSAND: {
      ...
      //else if (CMP_NTYPE(n->r, N_INDEX))
        ...
        switch (ret_type.type) {
          /*
  {
          //ReportMessage(lang_stat, n->t, "Cannot take address of static
  arrays. Try just indexing it, use the plain variable.");
  ...
  */
    ...
    case tkn_type2::T_MUL: {
      ...
      case enum_type2::TYPE_INT: {
        REPORT_ERROR(n->t->line, n->t->line_offset,
                     VAR_ARGS("Can't make type out a ptr number, try u32, s32 "
  ...
  case node_type::N_IDENTIFIER: {
    if (n->t->str == "global") {
      ...
      int f = (flags & NM_FND_TP_RETURN_EVEN_IDENT_NOT_DONE) *
              FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE;
      ...
      if (IS_FLAG_ON(flags, NM_FND_ASSIGN_FLAG_FOR_NODE_WHEN_DECL_NOT_DONE)) {
        n->flags |= NODE_FLAGS_RETURN_IDENT_EVEN_NOT_DONE;
      }
...
bool FuncArgsLogic(lang_state *lang_stat, func_decl *fdecl, node *fnode,
  ...
  FOR_VEC(t, args) {
    ...
    if (t->type == COMMA_VAR_ARGS) {
      ...
      */
      ...
      if (!TryInstantiateStruct(lang_stat, rel_ar->type.strct,
                                "rel_array_var_arg", child_scp, &rel_ar_var_arg,
                                templates, &var_arg->type))
        return false;
      dummy_type.strct = rel_ar_var_arg;
      auto new_decl = DeclareDeclToScopeAndMaybeToFunc(
          lang_stat, "__var_args", &dummy_type, child_scp, t->n);
      ...
      type2 aux_type;
      continue;
      // child_scp->vars.emplace_back(NewDecl(lang_stat, t->decl.name,
      // t->decl.type));
    }

    // decl2* new_decl = new decl2();
    memset(&dummy_type, 0, sizeof(type2));
    auto new_decl = DeclareDeclToScopeAndMaybeToFunc(lang_stat, "", &dummy_type,
                                                     child_scp, t->n);
    ...
    if (t->decl.type.type == TYPE_STRUCT &&
        IS_FLAG_ON(t->decl.type.strct->flags, TP_STRCT_TUPLE)) {
      new_decl->type.ptr = 1;
    }
    ...
    if (t->type != COMMA_RET_COLON && !CMP_NTYPE_BIN(t->n, T_EQUAL)) {
      ...
      bool function_allows_unamed =
          IS_FLAG_ON(fdecl->flags, flags) || fnode->type == N_FUNC_DEF;
      ...
      if (normal_function_unamed) {
        new_decl->name += own_std::to_string(rand() % 0xffffff);
  ...
  if (!is_outsider)
    fdecl->args.assign(child_scp->vars.begin() + template_end_idx,
                       child_scp->vars.end());

  if (normal_func_first_arg_decl_with_using)
    AddStructMembersToScopeWithUsing(
        lang_stat, normal_func_first_arg_decl_with_using->type.strct,
        fdecl->scp,
        NewIdentNode(lang_stat, normal_func_first_arg_decl_with_using->name,
                     fnode->t));
  // fdecl->vars.assign(child_scp->vars.begin() + template_end_idx,
  // child_scp->vars.end());
...
bool TemplatedStructMatches(lang_state *lang_stat, node *ncall, scope *scp,
                            type_struct2 *st_templ, type_struct2 *st_to_check) {
  type2 dummy_type;
  if (st_to_check->original_strct &&
      st_to_check->original_strct->name == st_templ->name) {
    // searching for templates and creating decls
    FOR_VEC(decl, st_to_check->scp->vars) {
      decl2 *d = *decl;
      if (d->type.type == TYPE_TEMPLATE) {
        decl2 *found_templ =
            FindIdentifier(d->name, lang_stat->aux_scp, &dummy_type);
        if (!found_templ) {
          decl2 *new_aux_temp =
              (decl2 *)AllocMiscData(lang_stat, sizeof(decl2));
          new_aux_temp->type = *d->type.tp;
          new_aux_temp->name = own_std::string(d->name);
          new_aux_temp->type.type = FromTypeToVarType(new_aux_temp->type.type);
          lang_stat->aux_scp->vars.emplace_back(new_aux_temp);
        } else {
          ASSERT(CompareTypes(d->type.tp, &found_templ->type));
        }
      }
    }
    return true;
...
bool TemplatedFuncMatchWithTypes(lang_state *lang_stat, node *ncall, scope *scp,
                                 own_std::vector<type2> *given_args,
                                 func_decl *fdecl) {
  type2 dummy_type;
  ...
  if (!lang_stat->aux_scp)
    lang_stat->aux_scp = NewScope(lang_stat, nullptr);

  FOR_VEC(d, lang_stat->aux_scp->vars) {
    __lang_globals.free(__lang_globals.data, *d);
  }
  lang_stat->aux_scp->ClearDecls();

  FOR_VEC(arg, fdecl->args) {
    decl2 *f_arg = *arg;
    type2 *cur_given = &(*given_args)[cur_given_arg];

    if (f_arg->type.type == TYPE_STRUCT &&
        f_arg->type.strct->templates.size() > 0) {
      if (!TemplatedStructMatches(lang_stat, ncall, lang_stat->aux_scp,
                                  f_arg->type.strct, cur_given->strct)) {
        return false;
      }
    } else if (f_arg->type.type == TYPE_TEMPLATE) {
      decl2 *templ_found = FindIdentifier(f_arg->type.e_decl->name,
                                          lang_stat->aux_scp, &dummy_type);
      ASSERT(templ_found);
      dummy_type = templ_found->type;
      dummy_type.ptr += f_arg->type.ptr;
      // templ_found->type.ptr = dummy_type.ptr;

      if (!CompareTypes(&dummy_type, cur_given))
        return false;

    } else if (!CompareTypes(&f_arg->type, cur_given))
      return false;
    cur_given_arg++;
  }
  return true;
}
bool CheckIfTemplateOverloadHasFunc(lang_state *lang_stat, node *ncall,
                                    scope *scp,
                                    own_std::vector<type2> *given_args,
                                    decl2 *overload_decl, func_decl **out) {
  func_overload_strct *overload = overload_decl->type.overload_funcs;
  func_decl *found = overload->cached.Get(ncall);
  if (found) {
    *out = found;
    return true;
  }
  ...
  FOR_VEC(_fdecl, overload->fdecls) {
    ...
    if (TemplatedFuncMatchWithTypes(lang_stat, ncall, scp, given_args, f)) {
      ...
      if (f->templates.size() == 0) {
        ...
        found_decl = FindIdentifier(fname, scp, &dummy_type,
                                    FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
      }
      ...
      if (!found_decl) {
        ...
        fdecl->from_overload = overload_decl;
        //
        ...
        overload->fdecls.emplace_back(fdecl);
        ...
        FOR_VEC(t, fdecl->templates) {
          ...
          cur_templ->type.type = FromVarTypeToType(cur_templ->type.type);
          fdecl->scp->vars.emplace_back(
              NewDecl(lang_stat, t->name, cur_templ->type));
          i++;
        }
        fdecl->this_decl = new_decl;
        // fdecl->scp->AssignDecls(templates_types.begin(),
        // templates_types.end());
...
bool AddNewTemplFuncFromLangArrayTemplTypesToScope(
    lang_state *lang_stat, own_std::string original_name, scope *scp,
    own_std::vector<type2> *final_types, node *ncall,
    func_decl **fdecl_out = nullptr) {
  type2 ret_type;
  ...
  func_decl *fdecl;
  own_std::string fname =
      FuncNameWithTempls(lang_stat, original_name, final_types);
  auto found_decl = FindIdentifier(fname, scp, &ret_type,
                                   FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
  if (!found_decl) {
    func_decl *src_fdecl = it_next->type.fdecl;
    if (it_next->type.type == TYPE_OVERLOADED_FUNCS) {
      // if (!CheckIfTemplateOverloadHasFunc(lang_stat, ncall, scp, final_types,
      // it_next->type.overload_funcs, &src_fdecl)) return false;
      ASSERT(0);
  ...
  if (it_next->type.type == TYPE_OVERLOADED_FUNCS) {
    // ASSERT(final_types->size() == fdecl->args.size())
  } else
  ...
  if (IS_FLAG_OFF(fdecl->flags, FUNC_DECL_TEMPLATES_DECLARED_TO_SCOPE)) {
    FOR_VEC(t, fdecl->templates) {
      templates_types.emplace_back(
          NewDecl(lang_stat, t->name, (*final_types)[i]));
      i++;
    }
  ...
  if (fdecl->args.size() == 0)
    fdecl->args.assign(fdecl->scp->vars.begin() + fdecl->templates.size(),
                       fdecl->scp->vars.end());

  // getting the scope vars
  // DescendNode(fdecl->func_node->r, fdecl->scp);
  if (fdecl->func_node->r &&
      !DescendNameFinding(lang_stat, fdecl->func_node->r->r, fdecl->scp))
    return false;
  ...
  if (fdecl_out)
    *fdecl_out = fdecl;
  fdecl->flags |= FUNC_DECL_IS_DONE | FUNC_DECL_INSTANTIATED;
  type2 tp;
  tp.type = TYPE_FUNC;
  tp.fdecl = fdecl;
  fdecl->templates.clear();
...
void ReportMessageOne(lang_state *lang_stat, token2 *tkn, char *msg,
                      void *data) {
  char msg_hdr[256];
  char final_msg[256];
...
void ReportTypeMismatch(lang_state *lang_stat, token2 *tkn, type2 *lhs,
                        type2 *rhs) {
  char msg_hdr[256];
  REPORT_ERROR(tkn->line, tkn->line_offset,
               VAR_ARGS("type mismatch. Expected %s, received %s\n\n",
                        TypeToString(*lhs).c_str(), TypeToString(*rhs).c_str()))
  ExitProcess(1);
}
void BuildMacroTree(lang_state *lang_stat, scope *scp, node *n,
...
node *CreateNewVarArgCall(lang_state *lang_stat, node *val, node *expr, int ptr,
                          node *ncall) {
  ...
  // casting to u64
  auto u64_nd =
      CreateNodeFromType(lang_stat, &lang_stat->u64_decl->type, ncall->l->t);
  auto casted = NewTypeNode(lang_stat, u64_nd, N_CAST, new_node(lang_stat, val),
                            ncall->t);
  casted->t = ncall->t;
  ...
  // arg 3
  auto tp_data_call =
      NewTypeNode(lang_stat, NewIdentNode(lang_stat, "get_type_data", ncall->t),
                  N_CALL, new_node(lang_stat, expr), ncall->t);
  tp_data_call->t = ncall->t;

  var_args_params.emplace_back(tp_data_call);

  // creating the final func call
  return MakeFuncCallArgs(lang_stat, "new_var_arg", nullptr, var_args_params,
                          ncall->l->t);
}
...
decl2 *CreateStructTuple(lang_state *lang_stat, node *n, scope *scp) {
  ...
  if (!tuple_exist) {
    ...
    type_struct2 *tstrct =
        (type_struct2 *)AllocMiscData(lang_stat, sizeof(type_struct2));
    ...
    int i = 0;
    FOR_VEC(c, commas) {
      decl2 *d = DeclareDeclToScopeAndMaybeToFunc(lang_stat, "", &c->decl.type,
                                                  child_scp, n);
      if (c->type == COMMA_RET_COLON) {
        d->name = c->decl.name;
      } else {
        d->name = own_std::string("t") + own_std::to_string(i);
      }
    ...
    decl2 *tuple_decl = DeclareDeclToScopeAndMaybeToFunc(lang_stat, tuple_name,
                                                         &dummy, dst_scp, n);
    ...
    tstrct->size =
        SetVariablesAddress(&tstrct->scp->vars, 0, &tstrct->biggest_type);
  } else
    ret = tuple_exist;
  return ret;
};
void CreateTemplateInstantiationName(own_std::string &name,
                                     own_std::vector<comma_ret> &args,
...
node *ContructTreeFromArgs(lang_state *lang_stat, node *args_nd,
                           own_std::vector<node *> &sorted_args, int idx) {
  if (idx == 0) {
    node *n = sorted_args[idx];
    memcpy(args_nd, n, sizeof(node));
    return args_nd;
  } else if (CMP_NTYPE_BIN(args_nd, T_COMMA)) {
    args_nd->l =
        ContructTreeFromArgs(lang_stat, args_nd->l, sorted_args, idx - 1);
    args_nd->r = sorted_args[idx];
    return args_nd;
  } else {
    node *nd = new_node(lang_stat, args_nd->t);
    node *ret = new_node(lang_stat, args_nd->t);
    // memcpy(ret, args_nd, sizeof(node));

    ret->t = (token2 *)AllocMiscData(lang_stat, sizeof(token2));
    ret->type = N_BINOP;
    ret->t->type = T_COMMA;
    ret->l = ContructTreeFromArgs(lang_stat, nd, sorted_args, idx - 1);

    ret->r = sorted_args[idx];
    return ret;
  }
}
void MaybeSortArgs(lang_state *lang_stat, node *ncall, func_decl *fdecl,
  ...
  FOR_VEC(t, *args) {
    if (t->type == COMMA_RET_EXPR && CMP_NTYPE_BIN(t->n, T_EQUAL)) {
      ...
      ASSERT(d)
      // args should only be assigned once
...
bool CallNode(lang_state *lang_stat, node *ncall, scope *scp, type2 *ret_type,
  ...
  struct defer_strct {
    ...
    bool can_do_stuff;
    defer_strct(scope *scp, func_decl *func, lang_state *l) {
      _scp = scp;
      f = func;
      can_do_stuff = false;
      lang_stat = l;
  ...
  */
  ...
  if (!decl_func) {
    if (ncall->l->type == N_FUNC_DECL) {
      ...
      lhs = &aux_decl;
    } else if (ncall->l->type == N_IDENTIFIER) {
      lhs = FindIdentifier(ncall->l->t->str, scp, &dummy_type,
                           FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
  ...
  // printf("on call %.*s, line %d\n", lhs->name.size(), lhs->name.data(),
  // ncall->t->line);
  if ((lhs->type.type == TYPE_FUNC || lhs->type.type == TYPE_FUNC_EXTERN) &&
      IS_FLAG_OFF(lhs->type.fdecl->flags,
                  FUNC_DECL_MACRO | FUNC_DECL_TEMPLATED)) {
    // scp->vars.insert(scp->vars.end(), lhs->type.fdecl->args.begin(),
    // lhs->type.fdecl->args.end());
    // scp->call_args.insert(scp->call_args.lhs->type.fdecl->args.begin(),
    // lhs->type.fdecl->args.end());
    scp->call_args.make_count(scp->call_args.size() + 1);
    own_std::vector<decl2 *> *ar = &scp->call_args.back();
    ar->clear();
    ar->insert(ar->end(), lhs->type.fdecl->args.begin(),
               lhs->type.fdecl->args.end());
    dfr.can_do_stuff = true;
  }
  ...
  if (ncall->r && !DescendNameFinding(lang_stat, ncall->r, scp)) {
    ...
    if (ncall->r->type != N_BINOP &&
        NameFindingGetType(
            ...
            NM_FND_TP_RETURN_EVEN_IDENT_NOT_DONE |
                NM_FND_ASSIGN_FLAG_FOR_NODE_WHEN_DECL_NOT_DONE)) {
      if (dummy_type.ptr > 0)
  ...
  if (ncall->r && IS_FLAG_OFF(lhs->type.fdecl->flags, FUNC_DECL_MACRO) &&
      !DescendNameFinding(lang_stat, ncall->r, scp) &&
      !rhs_type_not_done_but_its_ptr) {
    //	if (IS_FLAG_ON(scp->flags, SCOPE_INSIDE_FUNCTION))
    //		int last_ar_lit_sz = scp->fdecl->array_literal_sz;
    return false;
  }
  ...
  if (lang_stat->cur_func &&
      ...
      IS_FLAG_OFF(lhs->type.fdecl->flags,
                  FUNC_DECL_X64 | FUNC_DECL_MACRO | FUNC_DECL_INTERNAL)) {
    REPORT_ERROR(
        ncall->t->line, ncall->t->line_offset,
        VAR_ARGS(
            "function '%s' is x64, but the call to the function '%s' is not.",
  ...
  */
  ...
  case enum_type2::TYPE_FUNC: {
    ...
    if (!is_var_args) {
      ...
      if (required_args_count != args.size() &&
          lhs->type.type != TYPE_OVERLOADED_FUNCS &&
          IS_FLAG_OFF(lhs->type.fdecl->flags,
                      FUNC_DECL_HAS_DEFAULT_ARGUMENTS)) {
        ...
        REPORT_ERROR(ncall->t->line, ncall->t->line_offset,
                     VAR_ARGS("function call's  number of arguments dont "
                              "match. Expected %d, got %d\n\nHere's the func "
                              "definition: \n\n%s\n\nHere's the call:\n",
                              required_args_count, (int)args.size(),
                              GetFileLn(lang_stat, func_ln, from_fl)))
        ExitProcess(1);
      }
    }
    ...
    if (lhs->name == "get_type_data") {
      ...
      *ret_type = fdecl->ret_type;
      ...
      auto new_call =
          CreateNewVarArgCall(lang_stat, NewIntNode(lang_stat, 0, ncall->l->t),
                              ncall->r, ret_type->ptr, ncall);
      ...
      *ret_type = DescendNode(lang_stat, ncall, scp);
      ...
      if (!(args[0].decl.type.type != TYPE_ENUM)) {
        ReportMessage(lang_stat, ncall->r->t,
                      "enum_count only accepts enum as args");
        ExitProcess(1);
      }
      ...
      FOR_VEC(t, args) {
        ...
        // a tuple
          ...
          node *bin =
              NewTypeNode(lang_stat, nullptr, N_UNOP, struct_constr, ncall->t);
      ...
      if (lhs->type.type == enum_type2::TYPE_OVERLOADED_FUNCS) {
        FOR_VEC(t, args) { args_types.emplace_back(t->decl.type); }
        ...
        // if (!gotten_func)
        {
          ...
          {
                  REPORT_ERROR(ncall->t->line, ncall->t->line_offset,
                          VAR_ARGS("no overload found for func %s",
          lhs->name.c_str())
                  );
                  //ExitProcess(1);
                  return false;
          ...
          // else
          {
            ...
            // auto found_decl = FindIwdentifier(fname, scp, &dummy_type,
            // FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
            if (!CheckIfTemplateOverloadHasFunc(
                    lang_stat, ncall, scp, &args_types, lhs, &gotten_func)) {
            ...
            // own_std::string fname = FuncNameWithTempls(lang_stat, lhs->name,
            // &args_types2);
      ...
      if (fdecl->templates.size() > 0 &&
          IS_FLAG_OFF(fdecl->flags, FUNC_DECL_INSTANTIATED)) {
        scope *target_scope = fdecl->templates[0].scp;
        own_std::vector<type2> templ_types;

        if (is_templated_overload_so_no_need_to_get_templ_types) {
          templ_types = args_types;
          FOR_VEC(a, templ_types) { a->type = FromVarTypeToType(a->type); }
        } else {
          if (ncall->call_templates) {
            ...
            FOR_VEC(t, templ_ar) { templ_types.emplace_back(t->decl.type); }

          }
          // func call has templates specified
          else {
            // if(lhs->type.type == OVE)
            templ_types = TemplatesTypeToLangArray(lang_stat, &fdecl->templates,
                                                   &fdecl->args, &args);
        ...
        lang_stat->cur_file = fdecl->from_file;
        // own_std::string func_name = lhs->name;
        // if(lhs->type.type == TYPE_OVERLOADED_FUNCS)
        // func_name

        if (!AddNewTemplFuncFromLangArrayTemplTypesToScope(
                lang_stat, lhs->name, scp, &templ_types, ncall, &new_func)) {
      ...
      */
      ...
      for (int i = 0; i < fdecl->args.size(); i++) {
        ...
        if (comp_val && f_arg->type.type != TYPE_STRUCT &&
            f_arg->type.type != TYPE_ENUM &&
            f_arg->type.type != t->decl.type.type) {
          auto cast_tp_nd =
              CreateNodeFromType(lang_stat, &f_arg->type, ncall->t);
          auto casted = NewTypeNode(lang_stat, cast_tp_nd, N_CAST,
                                    new_node(lang_stat, t->n), ncall->t);
        ...
        if (!comp_val && f_arg->type.type == enum_type2::TYPE_STRUCT && f_arg->type.ptr == 0) 
        {
          ...
          if (f_arg->type.strct->constructors.size() == 0) {
            if (IS_FLAG_OFF(lang_stat->flags,PSR_FLAGS_REPORT_UNDECLARED_IDENTS))
              ...
              REPORT_ERROR(
                  ncall->t->line, ncall->t->line_offset,
                  VAR_ARGS("struct '%s' doesn't have any constructors for '%s'",
                           f_arg->type.strct->name.c_str(),
                           TypeToString(t->decl.type).c_str()));
              ExitProcess(1);
            }
          }

          auto constr = f_arg->type.strct->FindExistingOverload(
              lang_stat, &f_arg->type.strct->constructors, (void *)&f_arg->type,
              &tp_ar, true, ncall);
          if (!constr)
            return false;

          auto new_call = NewBinOpNode(
              lang_stat, NewIdentNode(lang_stat, constr->name, ncall->l->t),
              tkn_type2::T_MUL, t->n->NewTree(lang_stat));
          ...
          if (!comp_val) {
            REPORT_ERROR(t->n->t->line, t->n->t->line_offset,
                         VAR_ARGS("On call to '%s', argument %d type mismatch. "
                                  "Expected %s, received %s\n\n",
                                  fdecl->name.c_str(), i + 1,
                                  TypeToString(f_arg->type).c_str(),
                                  TypeToString(t->decl.type).c_str()))
            ExitProcess(1);
          }
          // ASSERT(comp_val)
      ...
      if (fdecl_arg_idx > 0) {
        ...
        if (scp->fdecl->total_of_var_args < total_var_args) {
          ...
          // scp->fdecl->call_strcts_val_sz +=
          // (diff)*var_arg_strct->type.strct->size;
  ...
  case enum_type2::TYPE_STRUCT_TYPE: {
    ...
    FOR_VEC(t, args) {
      if (t->type == COMMA_RET_IDENT) {
        ASSERT(FindIdentifier(t->decl.name, scp, &t->decl.type));
      } else if (t->type == COMMA_RET_EXPR && t->n->type == N_BINOP &&
                 t->n->t->type == T_COMMA) {
    ...
    if (lhs->type.strct->templates.size() != args.size()) {
      ...
      REPORT_ERROR(
          ncall->t->line, ncall->t->line_offset,
          VAR_ARGS("struct declaration's number of templates dont match. "
                   "Expected %d, got %d\n\nHere's the struct declaration "
                   "definition: \n\n%s\n\nHere's the call:\n",
                   (int)strct->templates.size(), (int)args.size(),
                   GetFileLn(lang_stat, strct->strct_node->t->line - 1,
                             strct->from_file)));
      ExitProcess(1);
    }
    ...
    if (IS_FLAG_ON(lang_stat->flags,
                   PSR_FLAGS_NO_INSTANTIATION_BUT_RET_STRCT) ||
        !rhs_type_not_done_but_its_ptr &&
            ((long long)type != 1 &&
             type->type.type == enum_type2::TYPE_TEMPLATE)) {
      ret_type->type = enum_type2::TYPE_STRUCT_TYPE;
    ...
    */
    ...
    if (IS_FLAG_OFF(lang_stat->flags,
                    PSR_FLAGS_DONT_CHANGE_TEMPL_STRCT_ND_NAME)) {
      /*
      ncall->l->FreeTree();
      ncall->r->FreeTree();
      ncall->l = nullptr;
...
void NewVarArgToScope(lang_state *lang_stat, scope *scp, type2 *tp,
                      func_decl *fdecl) {
  type2 aux_type;
  own_std::string str("array");
  ...
  type_struct2 *ret_strct;
  if (!TryInstantiateStruct(lang_stat, ar_decl->type.strct, sname, scp,
                            &ret_strct, dummy_arg, tp)) {
    ASSERT(false)
  }
...
void LookingForLabels(lang_state *lang_stat, node *n, scope *scp) {
  ...
  case N_FUNC_DECL: {
    ASSERT(0);
    FunctionIsDone(lang_stat, n, scp, &ret_type,
                   FUNCTION_IS_DONE_FLAGS_ONLY_DECLARE_SCOPE_AND_FUNC);
    if (n->r) {
...
bool FunctionIsDone(lang_state *lang_stat, node *n, scope *scp, type2 *ret_type,
                    int flags) {
  char msg_hdr[256];
  node *fnode = n;

  bool is_outsider = IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_LINK_NAME) ||
                     IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_OUTSIDER) ||
                     fnode->type == node_type::N_FUNC_DEF;
  ...
  if (fnode->l->l->l != nullptr && fdecl->templates.size() == 0) {
    ...
    auto tdecls = DescendTemplatesToDecl(lang_stat, fnode->l->l->l->r,
                                         child_scp, &fdecl->templates);
    child_scp->AssignDecls(tdecls.begin(), tdecls.end());
    template_end_idx = child_scp->vars.size();
  }

  fdecl->flags |= IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_OUTSIDER)
                      ? FUNC_DECL_IS_OUTSIDER
                      : 0;

  fdecl->flags |= IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_CONSTRUCTOR)
                      ? FUNC_DECL_CONSTRUCTOR
                      : 0;

  fdecl->flags |= IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_INTERNAL)
                      ? FUNC_DECL_INTERNAL
                      : 0;
  fdecl->flags |= IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_INTRINSIC)
                      ? FUNC_DECL_INTRINSIC
                      : 0;
  fdecl->flags |=
      IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_MACRO) ? FUNC_DECL_MACRO : 0;
  fdecl->flags |= IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_COROUTINE)
                      ? FUNC_DECL_COROUTINE
                      : 0;
  fdecl->flags |=
      IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_COMP) ? FUNC_DECL_COMP : 0;
  fdecl->flags |=
      IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_TEST) ? FUNC_DECL_TEST : 0;
  fdecl->flags |= IS_FLAG_ON(fnode->flags, NODE_FLAGS_ALIGN_STACK_WHEN_CALL)
                      ? FUNC_DECL_ALIGN_STACK_WHEN_CALL
                      : 0;
  fdecl->flags |=
  ...
  if (IS_FLAG_OFF(flags, DONT_DESCEND_ARGS) && fnode->l->l->r != nullptr &&
    ...
    if (!FuncArgsLogic(lang_stat, fdecl, fnode, child_scp, ret_type, flags,
                       template_end_idx, is_outsider))
      return false;
  }
  ...
  if (IS_FLAG_ON(fdecl->flags, FUNC_DECL_COROUTINE)) {
    ...
    if (!fdecl->coroutine_prologue_tree) {
      fdecl->coroutine_prologue_tree =
          d->type.fdecl->func_node->r->r->NewTree(lang_stat);
      BuildMacroTree(lang_stat, child_scp, fdecl->coroutine_prologue_tree,
                     n->t->line);
    }
  ...
  if (IS_FLAG_OFF(flags, DONT_DESCEND_SCOPE) &&
    ...
    // scope
    if (fnode->r != nullptr &&
        !DescendNameFinding(lang_stat, fnode->r, child_scp))
      return false;

    // fdecl->vars.insert(fdecl->vars.end(), child_scp->vars.begin() +
    // args_end_idx, child_scp->vars.end());
  }
  ...
  if (IS_FLAG_ON(fnode->flags, NODE_FLAGS_FUNC_TEST)) {
    if (fdecl->ret_type.type != enum_type2::TYPE_S32) {
      REPORT_ERROR(fnode->t->line, fnode->t->line_offset,
                   VAR_ARGS("test function must return s32:\n"));
    }
  }

  if (IS_FLAG_ON(fdecl->flags, FUNC_DECL_COMP) &&
      lang_stat->comp_time_funcs.find(fdecl) ==
          lang_stat->comp_time_funcs.end()) {
...
import_strct *NewImport(lang_state *lang_stat, import_type type,
                        own_std::string alias, unit_file *fl) {
...
node *NewUnopNode(lang_state *lang_stat, node *lhs, tkn_type2 t, node *rhs) {
  ...
  un->t = new_t;
  if (lhs) {
    un->t->line = lhs->t->line;
    un->l = lhs;
  } else {
    un->t->line = rhs->t->line;
    un->r = rhs;
  }
...
node *NewTypeNode(lang_state *lang_stat, node *lhs, node_type nd, node *rhs,
                  token2 *t) {
...
void BottomOfTheTreeInsertLhs(lang_state *lang_stat, node *n, node *insert) {
  ...
  auto new_n = NewBinOpNode(lang_stat, insert, tkn_type2::T_POINT,
...
decl2 *PointLogic(lang_state *lang_stat, node *n, scope *scp, type2 *ret_tp) {
  ...
  if (!lhs_decl) {
    if (IS_PRS_FLAG_ON(PSR_FLAGS_REPORT_UNDECLARED_IDENTS)) {
      REPORT_ERROR(n->l->t->line, n->l->t->line_offset,
                   VAR_ARGS("'%s' not found", n->l->t->str.c_str()));
      // ExitProcess(1);
    }
  ...
  // when lhs is 1, it means that a builtin type was returned, if the lhs_tp
  // isn't a struct lhs_tp gets a struct value if the n->l is a function call
  // for example,
  if ((long long)lhs_decl == 1 &&
      (lhs_tp.type != enum_type2::TYPE_VECTOR &&
       lhs_tp.type != enum_type2::TYPE_STRUCT_TYPE &&
       lhs_tp.type != enum_type2::TYPE_STRUCT))
    ASSERT(false)
  ...
  // getting how many lhs should be saved when we encounter another pointer that
  // also need to be dereferenced
  ...
  case enum_type2::TYPE_STATIC_ARRAY: {
    if (n->r->t->str == "len") {
      ...
      ReportMessageOne(
          lang_stat, n->r->t,
          "'%s' is not part of array literals:", (void *)n->r->t->str.c_str());
      ExitProcess(1);
  ...
  case enum_type2::TYPE_STRUCT_TYPE: {
    ...
    // ReportMessageOne(lang_stat, n->r->t, "struct type '%s' is not a variable
    // that can be indexed:", (void*)n->l->t->str.c_str());
  } break;
  case enum_type2::TYPE_VECTOR:
  case enum_type2::TYPE_STRUCT: {
    if (n->r->type == N_SCOPE) {
      ...
      AddStructMembersToScopeWithUsing(lang_stat, lhs->type.strct, n->r->scp,
                                       n->l);
      if (!DescendNameFinding(lang_stat, n->r, scp))
        return nullptr;
    ...
    // maybe modifying the tree for members that are being used with "using"
    // keyword
    auto ret = DescendNameFinding(lang_stat, n->r, lhs->type.strct->scp);
    // correcting the tree that was modified
    if (n->r->type == node_type::N_BINOP &&
        n->r->t->type == tkn_type2::T_POINT) {
      type2 ret_type;
    ...
    if (strct->original_strct && strct->original_strct->name == "rel_ptr") {
      ...
      auto op_func =
          lhs->type.strct->FindOpOverload(lang_stat, overload_op::DEREF_OP, n);
      ASSERT(op_func)
      *ret_tp = ret->type;
...
own_std::string FuncNameWithTempls(lang_state *lang_stat, own_std::string fname,
                                   own_std::vector<type2> *types) {
  own_std::string ret;
  ret += fname;
  ret += "_";
...
decl2 *DeclareDeclToScopeAndMaybeToFunc(lang_state *lang_stat,
                                        own_std::string name, type2 *tp,
                                        scope *scp, node *nd) {
  /*
  if (name == "entry")
          auto a = 0;
  ...
  // we only want to declare functions and structs. We're not interested in its
  // contents
  ...
  if (IS_FLAG_OFF(lang_stat->flags, PSR_FLAGS_DONT_ADD_TO_FUNC_VARS) &&
      IS_FLAG_ON(scp->flags, SCOPE_INSIDE_FUNCTION))
    scp->fdecl->vars.emplace_back(new_decl);

  // adding the data sect size
  if (IS_FLAG_ON(scp->flags, SCOPE_IS_GLOBAL) ||
      IS_FLAG_ON(tp->flags, TYPE_IS_GLOBAL)) {
    if (CanAddToDataSect(new_decl->type.type)) {
      new_decl->flags |= DECL_IS_GLOBAL;
      new_decl->offset =
          AddDataSectSizeRetPrevSize(lang_stat, GetTypeSize(&new_decl->type));
    }
  }

  return new_decl;
}
own_std::string MangleFuncNameWithArgs(lang_state *lang_stat, func_decl *fdecl,
                                       own_std::string original_name,
                                       int start_arg) {
  own_std::string name;
  own_std::vector<type2> args_types;
...
node *CreateNodeFromType(lang_state *lang_stat, type2 *tp, token2 *t) {
  ...
  // if the type of the array is a builting, we have the create a new node with
  // the type
  else {
...
bool TransformSingleFuncToOvrlStrct(lang_state *lang_stat, decl2 *decl_exist) {
  ...
  decl_exist->type.overload_funcs = (func_overload_strct *)AllocMiscData(
      lang_stat, sizeof(func_overload_strct));
  ...
  if (IS_FLAG_OFF(f->flags,
                  FUNC_DECL_TEMPLATED) /* && f->op_overload == COND_EQ_OP*/) {
    ...
    decl_exist->type.overload_funcs->templated = true;
    // f->name = MangleFuncNameWithArgs(lang_stat, f, f->name, 0);
  }
  decl_exist->type.overload_funcs->ovrld_op = f->op_overload;
...
node *CreateDeclNode(lang_state *lang_stat, own_std::string name, type2 *tp,
                     token2 *t) {
  auto new_decl = NewBinOpNode(lang_stat, NewIdentNode(lang_stat, name, t),
                               T_COLON, CreateNodeFromType(lang_stat, tp, t));

  return new_decl;
}
node *CreateDeclNode(lang_state *lang_stat, own_std::string name,
                     enum_type2 type, token2 *t) {
  type2 tp_;
  memset(&tp_, 0, sizeof(type2));
  tp_.type = type;
  return CreateDeclNode(lang_stat, name, &tp_, t);
}
node *CreateBinOpRhsBool(lang_state *lang_stat, node *lhs, keyword kw,
                         tkn_type2 bin) {
...
node *CreateBoolExpression(lang_state *lang_stat, node *var, node *expr,
                           scope *scp) {
  /*
...
node *NewThreeArgNd(lang_state *lang_stat, own_std::string name, node *arg1,
                    node *arg2, node *arg3) {
  own_std::vector<node *> args;
...
/*
node* NewThreeArgNd2(own_std::string name, void* arg1, int arg1_type, void*
arg2, int arg2_type, void* arg3, int arg3_type)
{
...
#define NEW_THREE_ARG_NDFLAGS_I_I(name, arg1, flags1, arg2, arg3)              \
  NewThreeArgNd2(name, (void *)arg1, PROCESS_ND_IS_ND | (flags1),              \
                 (void *)arg2, PROCESS_ND_IS_INT, (void *)arg3,                \
                 PROCESS_ND_IS_INT);

#define NEW_BIN_OP_ND_I(nd, tkn, i)                                            \
  NewBinOpNode(lang_stat, nd, tkn, NewIntNode(lang_stat, i, nd->t));

void CheckDeclNodeAndMaybeAddEqualZero(lang_state *lang_stat, node *n,
                                       scope *scp) {
  if (IsNodeOperator(n, T_COLON) &&
      IS_FLAG_ON(scp->flags, SCOPE_INSIDE_FUNCTION) &&
      n->r->type != N_STRUCT_DECL &&
      IS_FLAG_OFF(
          n->flags,
          NODE_FLAGS_NO_ZERO_INITIALIZATION) //&& IS_FLAG_ON(n->flags,
                                             // NODE_FLAGS_IS_PROCESSED2)
  ) {
    type2 dummy_tp;
    decl2 *decl = FindIdentifier(n->l->t->str, scp, &dummy_tp);

    if (decl->type.type == TYPE_STATIC_ARRAY ||
        IS_FLAG_ON(decl->flags, DECL_IS_GLOBAL))
      return;
    ...
    if (decl->type.type == TYPE_STRUCT && decl->type.ptr == 0) {
      ...
      final_nd = NewThreeArgNd(
          ...
          NewIntNode(lang_stat, decl->type.strct->size, n->t));
    ...
    auto stmnt_nd = NewTypeNode(lang_stat, new_node(lang_stat, n),
                                node_type::N_STMNT, final_nd, n->t);
    if (lang_stat->is_lsp) {
...
void EnumToTypeSect(lang_state *lang_stat, own_std::string enum_name,
                    scope *scp) {
  ...
  InsertIntoCharVector(&strct_str_tbl, (void *)enum_name.data(),
                       enum_name.size());
  ...
  FOR_VEC(vr, vars) {
    ...
    InsertIntoCharVector(&strct_str_tbl, (void *)v->name.data(),
                         v->name.size());
  ...
  type_sect->insert(type_sect->end(), strct_str_tbl.begin(),
                    strct_str_tbl.end());
}

void AddStructMembersToScopeWithUsing(lang_state *lang_stat,
                                      type_struct2 *strct, scope *scp,
                                      node *by_name_nd) {
  ...
  FOR_VEC(v, strct->scp->vars) {
    ...
    if ((*v)->using_node == nullptr) {
      ...
      new_decl->using_node = NewBinOpNode(
          lang_stat, new_node(lang_stat, by_name_nd), tkn_type2::T_POINT,
    ...
    // assigning the using_n to the top of the tree
      ...
      BottomOfTheTreeInsertLhs(lang_stat, new_decl->using_node,
...
node *MakeMemCpyCall(lang_state *lang_stat, node *lhs, node *rhs, node *top,
                     int size) {
  ...
  lang_stat->void_decl->type.ptr++;
  auto void_nd =
      CreateNodeFromType(lang_stat, &lang_stat->void_decl->type, lhs->t);
  ...
  auto call_nd = NewThreeArgNd(lang_stat, "memcpy", casted1, casted2,
                               NewIntNode(lang_stat, size, lhs->t));
  if (lang_stat->is_lsp) {
...
void GetFilesInDirectory(own_std::string dir, own_std::vector<char *> *contents,
                         own_std::vector<char *> *file_names, int flags = 0) {
  bool recursive = IS_FLAG_ON(flags, GET_FILES_DIR_RECURSIVE);
  bool add_path_to_name =
      IS_FLAG_ON(flags, GET_FILES_DIR_ADD_PATH_TO_FILE_NAME);
#ifdef LINUX
  ...
  while ((entry = readdir(dir_info)) != NULL) {
    ...
    if (add_path_to_name) {
      ...
      auto buffer =
          (char *)heap_alloc((mem_alloc *)__lang_globals.data, sz + 1);
...
bool NameFindingOnExprEtruct(lang_state *lang_stat, type_struct2 *strct,
                             node *n, scope *scp, bool create_point_binop) {
  ...
  if (n->type == N_BINOP) {
    ...
    if (!NameFindingOnExprEtruct(lang_stat, strct, n->l, scp,
                                 create_point_binop))
      return false;
    if (!NameFindingOnExprEtruct(lang_stat, strct, n->r, scp,
                                 create_point_binop))
      return false;
    ...
    if (create_point_binop) {
      node *d =
          NewBinOpNode(lang_stat, NewIdentNode(lang_stat, strct->name, n->t),
                       tkn_type2::T_POINT, new_node(lang_stat, n));
...
/*
bool CheckForConstDecls(lang_state *lang_stat, node *n, func_decl* fdecl, scope*
scp)
{
        if(!n)
                return true;
...
*/
  case node_type::N_BINOP: {
    ...
    if (n->l && CMP_NTYPE_BIN(n->l, T_COLON) && n->l->r &&
        n->l->r->type == N_CONST) {
  ...
  */
  ...
  case N_ON: {
    ...
    if (ret_type.type == TYPE_STRUCT &&
        IS_FLAG_OFF(n->flags, NODE_FLAGS_IS_PROCESSED)) {
      ...
      FOR_VEC(cur_cond, on->exprs) {
        NameFindingOnExprEtruct(lang_stat, ret_type.strct, cur_cond->cond, scp,
                                true);
      }
    } else {
      FOR_VEC(cur_cond, on->exprs) {
  ...
  case N_ASSIGN_SER_FUNC: {
    if (IS_FLAG_ON(n->flags, NODE_FLAGS_IS_PROCESSED2))
      return (decl2 *)1;
    ASSERT(CMP_NTYPE_BIN(n->r, T_COMMA));
    ASSERT(n->r->r->type == N_IDENTIFIER);

    decl2 *lhs = DescendNameFinding(lang_stat, n->r->l, scp);
    decl2 *rhs = DescendNameFinding(lang_stat, n->r->r, scp);
    ...
    ASSERT(n->r->l->type == N_IDENTIFIER);

    lhs = FindIdentifier(n->r->l->t->str, scp, &ret_type);
    rhs = FindIdentifier(n->r->r->t->str, scp, &ret_type);

    ASSERT(lhs->type.type == TYPE_STRUCT_TYPE);
    ASSERT(rhs->type.type == TYPE_FUNC);

    lhs->type.strct->ser_func = rhs;

    int offset = lhs->type.strct->type_sect_offset;
    type_data *tp = (type_data *)&lang_stat->type_sect[offset];

    tp->serializable_func = rhs;

    lang_stat->ser_funcs_type_data.emplace_back(offset);
    n->flags |= NODE_FLAGS_IS_PROCESSED2;
  ...
  case N_SERIALIZABLE: {
    ...
    FOR_VEC(c, commas) {
      switch (c->type) {
      case COMMA_RET_EXPR: {
        ASSERT(CMP_NTYPE_BIN(c->n, T_EQUAL));
        ASSERT(c->n->r->type == N_IDENTIFIER);
        if (c->n->l->t->str == "ser_func") {
          decl2 *d = FindIdentifier(c->n->r->t->str, scp, &ret_type);
          if (!d) {
            if (IS_FLAG_ON(lang_stat->flags,
                           PSR_FLAGS_REPORT_UNDECLARED_IDENTS)) {
              ReportUndeclaredIdentifier(lang_stat, n->t);
          ...
          ASSERT(d->type.type == TYPE_FUNC);

          ASSERT(scp->type == SCP_TYPE_STRUCT);

          scp->tstrct->ser_func = d;
        } else {
          ASSERT(0);
        }
      } break;
  ...
  case node_type::N_KEYWORD: {
    ...
    case KW_USING: {
      ...
      AddStructMembersToScopeWithUsing(lang_stat, decl->type.strct, scp,
                                       by_name_nd);
  ...
  case node_type::N_IMPORT: {
    if (n->r->type == node_type::N_BINOP && n->r->t->str == "as") {
      ...
      own_std::string alias = n->r->r->t->str;
      own_std::string imp_name = n->r->l->t->str;
      ...
      bool was_added = false;
      own_std::string file_name = n->r->t->str;
      FOR_VEC(dcl, scp->imports) {
  ...
  case node_type::N_FUNC_DECL: {
    if (IS_FLAG_ON(n->flags, NODE_FLAGS_FUNC_CONSTRUCTOR)) {
      ...
      // getting args types, because their type names will be the name of the
      // construct
      own_std::vector<type2> tps;
      FOR_VEC(a, n->fdecl->args) { tps.emplace_back((*a)->type); }

      // ar[i..0];
      own_std::string fname = FuncNameWithTempls(
          lang_stat,
          own_std::string("constr_") + TypeToString(n->fdecl->ret_type), &tps);
      ...
      DeclareDeclToScopeAndMaybeToFunc(lang_stat, n->fdecl->name, &ret_type,
                                       scp, n);
      n->flags |= NODE_FLAGS_IS_PROCESSED2;
    } else {
      if (!FunctionIsDone(lang_stat, n, given_scp, &ret_type, 0))
    ...
    if (IS_FLAG_OFF(n->flags, NODE_FLAGS_INDEX_IS_TYPE)) {
      ...
      if (n->r->type == N_BINOP && n->r->t->type == T_TWO_POINTS) {
        ...
        if (lhs->type.type == enum_type2::TYPE_STRUCT) {
          ...
          data_nd = NewBinOpNode(lang_stat, new_n, tkn_type2::T_POINT,
                                 NewIdentNode(lang_stat, "data", n->t));

          auto len_nd = NewBinOpNode(lang_stat, new_n, tkn_type2::T_POINT,
        ...
        node *call_nd =
            MakeFuncCallArgs(lang_stat, "slice_array", nullptr, args, n->t);
        call_nd->t = n->t;
        memcpy(n, call_nd, sizeof(node));

        // creating specified template types for the call
        //
        // if the type of the array is a struct we simply add a ident nd as
        // templ type
        ...
        if (!lhs_type.IsStrct(nullptr) && lhs_type.type != TYPE_STATIC_ARRAY) {
          if (lhs_type.ptr == 0) {
            if (lhs_type.type == TYPE_STR_LIT) {
              ...
              auto tp_nd = CreateNodeFromType(
                  lang_stat, &lang_stat->char_decl->type, n->t);
              args.emplace_back(tp_nd);

              node *ncall = MakeFuncCallArgs(lang_stat, "ptr_offset", nullptr,
                                             args, n->t);
              ncall->t = n->t;
  ...
  case node_type::N_IDENTIFIER: {
    ...
    */
    auto ident = FindIdentifier(n->t->str, scp, &ret_type);
    if (ident &&
        IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_DECLARE_ONLY_TYPE_PARAMTS)) {
  ...
  case node_type::N_OP_OVERLOAD: {
    ...
    // a strct ptr to struct, in case of templated strct, a ptr to the original
    // one so that structs that were devired from that one can query its
    // "mother" struct for ovoerloaded funcs
  ...
  case node_type::N_BINOP: {
    ...
    case tkn_type2::T_AT: {
      ...
      node *ref_nd = NewBinOpNode(lang_stat, nullptr, tkn_type2::T_AMPERSAND,
                                  new_node(lang_stat, n->l));
    ...
    case tkn_type2::T_EQUAL: {
      decl2 *lhs;
      if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_ON_FUNC_CALL)) {
        scp->flags |= SCOPE_RETURN_CALL_ARGS_FROM_SCP;
      ...
      if ((long long)lhs != 1) {
        if (lhs->type.is_const) {
          ...
          bool is_str_lit = lhs->type.type == enum_type2::TYPE_STR_LIT &&
                            lhs->type.tp == nullptr;
          // type inference
          if (lhs->type.type == enum_type2::TYPE_AUTO) {
            ...
            if (NameFindingGetType(lang_stat, n->r, scp, lhs->type)) {
              if (lhs->type.type == enum_type2::TYPE_STRUCT_TYPE)
                ...
                ReportMessage(lang_stat, n->t,
                              "function definition is not a value, so it cant "
              ...
              if (lhs->type.type == TYPE_VOID && lhs->type.ptr == 0) {
                ReportMessage(lang_stat, n->t,
                              "cannot assign plain void to variables\n");
              }
              n->l->r = CreateNodeFromType(lang_stat, &lhs->type, n->t);
            ...
            if (is_str_lit) {
              ...
              bool val = InstantiateArFromType(lang_stat, u8_tp, scp,
                                               &lhs->type, &ret_struct, n);
              ASSERT(val)
          ...
          // if we have a var decl before the assignment, and the decl is a
          // struct, we have to split the assignment and the decl into two
          // different stmnts
          if (IsNodeOperator(n->l, tkn_type2::T_COLON)) {
            ...
            // lhs = rhs;
            auto new_equal =
                NewBinOpNode(lang_stat, n->l->l, tkn_type2::T_EQUAL,
                             new_node(lang_stat, n->r));
          ...
          bool is_struct_val = (lhs->type.type == TYPE_STRUCT) &&
                               lhs->type.ptr == 0 &&
                               IsNodeUnop(equal_stmnt->r, T_MUL);
          if (lhs->type.type == TYPE_STRUCT && lhs->type.ptr == 0 &&
              !zero_initialization) {
          }

          if (is_bool && IS_FLAG_OFF(n->flags, NODE_FLAGS_IS_PROCESSED2) &&
              equal_stmnt->r->type == N_BINOP &&
              equal_stmnt->r->t->type != T_POINT) {
            node *bool_expr = CreateBoolExpression(lang_stat, equal_stmnt->l,
                                                   equal_stmnt->r, scp);
            ...
            auto arg2 =
                NewUnopNode(lang_stat, nullptr, T_AMPERSAND, equal_stmnt->r);

            lang_stat->void_decl->type.ptr++;
            auto void_nd = CreateNodeFromType(
                lang_stat, &lang_stat->void_decl->type, n->t);
            ...
            auto call_nd = NewThreeArgNd(
                lang_stat, "memcpy", casted1, casted2,
                NewIntNode(lang_stat, lhs->type.strct->size, n->t));
            if (lang_stat->is_lsp) {
              call_nd->modified = true;
    ...
    //%COLON
    case tkn_type2::T_COLON: {
      const own_std::string &decl_name =
          n->l != nullptr ? n->l->t->str : own_std::to_string((long long)n);
      ...
      if (n->l == nullptr) {
        ...
        n->l->t->str = own_std::to_string((long long)n);
        // auto decl_exist = FindIdentifier(decl_name, scp, &ret_type,
        // FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
      }

      auto decl_exist =
          FindIdentifier((own_std::string &)decl_name, scp, &ret_type,
                         FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE);
          ...
          decl_exist->decl_nd != n)
      {
        ...
        if (decl_exist->decl_nd->type == N_OP_OVERLOAD) {
          ReportMessage(lang_stat, n->t,
                        "At the moment we only allow overloading of operators");
          ExitProcess(1);
        }
      ...
      if (decl_exist && IS_FLAG_OFF(lang_stat->flags,
                                    PSR_FLAGS_IGNORE_DECL_EXIST_AND_DECLARE)) {
        if (decl_exist->type.type == enum_type2::TYPE_OVERLOADED_FUNCS)
          ...
          if (decl_exist->decl_nd != n) {
            ...
            same name as this one\n", decl_exist->name.c_str(), decl_exist_ln,
            GetFileLn(decl_exist_ln - 1)
                    )
            )
            */
            ExitProcess(1);
      ...
      if (n->r) {
        ...
        case node_type::N_CALL: {
          ...
          if (!CallNode(lang_stat, n->r, scp, &ret_type))
            return nullptr;
          if (decl_exist && decl_exist->type.type == TYPE_OVERLOADED_FUNCS &&
              ret_type.type != TYPE_FUNC) {
            REPORT_ERROR(
                n->t->line, n->t->line_offset,
                VAR_ARGS("'%s' is a function, declared here:\n\n%s\n\nbut you "
                         "are trying to overload it with something that is not "
                         "a function\n",
                         prev_decl->name.c_str(),
                         GetFileLn(lang_stat, prev_decl->decl_nd->t->line - 1,
                                   prev_decl->from_file)));
            ExitProcess(1);
          }
        ...
        case node_type::N_UNOP: {
          ...
          bool is_ret_nil =
              IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_DONT_DECLARE_VARIABLES);
          int flags_to_get_type =
              was_ptr * NM_FND_TP_RETURN_EVEN_IDENT_NOT_DONE * !is_ret_nil;
          // DescendNameNM_FND_TP_RETURN_EVEN_IDENT_NOT_DONE
          if (!NameFindingGetType(lang_stat, n->r, scp, ret_type,
                                  flags_to_get_type)) {
            bool found = false;
            if (NameFindingGetType(
                    lang_stat, n->r, scp, ret_type,
                    flags_to_get_type |
                        NM_FND_ASSIGN_FLAG_FOR_NODE_WHEN_DECL_NOT_DONE)) {
              if (ret_type.ptr > 0)
                found = true;
          ...
          if (ret_type.type == TYPE_STRUCT_TYPE &&
              ret_type.strct->templates.size() > 0 &&
              IS_FLAG_OFF(ret_type.flags, TYPE_NOT_INSTANTIATED_YET)) {
            auto strct = ret_type.strct;
            REPORT_ERROR(
                n->r->t->line, n->r->t->line_offset,
                VAR_ARGS("This struct declaration must have templates! Check "
                         "out its header:\n\n %s\n",
                         GetFileLn(lang_stat, strct->strct_node->t->line - 1,
                                   strct->from_file)));
            ExitProcess(1);
        ...
        case node_type::N_FUNC_DEF: {
          ...
          lang_stat->flags |= PSR_FLAGS_DECLARE_ONLY_TYPE_PARAMTS;
          if (!FunctionIsDone(lang_stat, n->r, scp, &ret_type,
                              DONT_DESCEND_SCOPE)) {
            lang_stat->flags &= ~PSR_FLAGS_DECLARE_ONLY_TYPE_PARAMTS;
            return nullptr;
          }
        ...
        case node_type::N_ENUM_DECL: {
          ...
          if (!decl_exist) {
            ...
            decl_exist = DeclareDeclToScopeAndMaybeToFunc(lang_stat, decl_name,
                                                          &ret_type, scp, n);
          ...
          FOR_VEC(n, names) {
            ...
            case COMMA_RET_EXPR: {
              if (!IsNodeOperator(n->n, tkn_type2::T_EQUAL))
                ReportMessage(lang_stat, n->n->t,
                              "Only assignment expression is allowed for enum");

              // rhs must be know at compile time
              auto rhs_tp = DescendNode(lang_stat, n->n->r, child_scp);

              if (rhs_tp.type != enum_type2::TYPE_INT &&
                  rhs_tp.type != enum_type2::TYPE_ENUM_IDX_32)
                ReportMessage(lang_stat, n->n->r->t,
                              "Value must be know at compile time");

              cur_idx = GetExpressionVal(n->n->r, child_scp);
              new_decl->name = n->n->l->t->str;
            } break;
            case COMMA_RET_IDENT: {
              new_decl->name = n->decl.name;
            } break;
            default:
              ASSERT(false)
            }
          ...
          own_std::string final_name =
              own_std::string("$$enum ") + decl_exist->name;
          // inserting an symbol for the type
          //
          lang_stat->type_sect_syms.emplace_back(
              machine_sym(lang_stat, SYM_TYPE_DATA,
                          (unsigned int)lang_stat->type_sect.size(),
                          (char *)std_str_to_heap(lang_stat, &final_name)));
        } break;
        ...
        case node_type::N_STRUCT_DECL: {
          ...
          // snode->tstrct = snode->tstrct == nullptr ? new type_struct2() :
          // snode->tstrct;
          ...
          if (snode->l == nullptr) {
            if (n->r->type == N_ETRUCT_DECL &&
              ...
              if (!commas) {
                ...
                n->r->exprs = (own_std::vector<comma_ret> *)AllocMiscData(
                    lang_stat, sizeof(own_std::vector<comma_ret>));
                commas = n->r->exprs;
                node *descend = n->r->r;
              ...
              FOR_VEC(c, *commas) {
                if (c->n->type == N_CALL) {
                  if (!c->n->tstrct)
                    c->n->tstrct = (type_struct2 *)AllocMiscData(
                        lang_stat, sizeof(type_struct2));
                  type_struct2 *cur_strct = c->n->tstrct;
                  if (!c->n->r) {
                    c->n->r = new_node(lang_stat, c->n->t);
                  }
                  scope *new_struct_scp =
                      GetScopeFromParent(lang_stat, c->n->r, child_scp);
              ...
              for (int cidx = commas->size() - 1; cidx >= 0; cidx--) {
                ...
                if (c->n->type == N_CALL) {
                  ...
                  decl2 *new_decl_type =
                      NewDecl(lang_stat, name + "__type__", ret_type);
                  ...
                  c->n->tstrct->scp->tstrct->name =
                      own_std::string(new_decl_type->name);
                  type_struct2 *st = c->n->tstrct;
                  st->size =
                      SetVariablesAddress(&st->vars, 0, &st->biggest_type);
                  // struct_types
                  ...
                  c->n->tstrct->ToTypeSect(lang_stat, &lang_stat->type_sect,
                                           &lang_stat->type_sect_str_tbl_sz);

                  tstrct->biggest_type =
                      max(tstrct->biggest_type, st->biggest_type);
                } else {
                  name = c->n->t->str;
              ...
              if (!DescendNameFinding(lang_stat, snode->r->r, child_scp)) {
                ...
                lang_stat->cur_file->arrived_at_node->t->line,
                n->l->t->str.c_str()); DescendNameFinding(lang_stat,
                snode->r->r, child_scp);
            ...
            FOR_VEC(v, tstrct->vars) {
              ...
              if (IsSameStrct(vtp, tstrct) ||
                  (vtp->type == TYPE_STATIC_ARRAY &&
                   IsSameStrct(vtp->tp, tstrct) && vtp->ptr == 0)) {
                ReportMessage(lang_stat, (*v)->decl_nd->t,
                              "Cannot self declare the struct");
                ExitProcess(1);
              }
            }
            ...
            if (n->r->type == node_type::N_STRUCT_DECL)
              tstrct->size =
                  SetVariablesAddress(&tstrct->vars, 0, &tstrct->biggest_type);
              ...
              FOR_VEC(v, child_scp->vars) {
                if ((*v)->type.type == TYPE_STRUCT_TYPE)
                  continue;
                (*v)->offset = type_var_sz;
                int cur_vsize = GetTypeSize(&(*v)->type);
                if (cur_vsize > biggest_var)
                  biggest_var = cur_vsize;
                tstrct->biggest_type =
                    CheckBiggestType(&(*v)->type, tstrct->biggest_type);
              }
              tstrct->size = biggest_var + type_var_sz;
          ...
          if (decl_exist->type.type == TYPE_STRUCT_TYPE &&
              decl_exist->type.strct->templates.size() == 0) {
            decl_exist->type.strct->ToTypeSect(
                lang_stat, &lang_stat->type_sect,
                &lang_stat->type_sect_str_tbl_sz);
            own_std::string final_name =
                own_std::string("$$") + decl_exist->type.strct->name;
            // inserting an symbol for the type
            //
            lang_stat->type_sect_syms.emplace_back(
                machine_sym(lang_stat, SYM_TYPE_DATA,
        ...
        case node_type::N_BINOP: {
          ...
          case tkn_type2::T_COLON: {
            ...
            switch (cnode->type) {
            case node_type::N_STRUCT_DECL: {
              // cnode->tstrct = cnode->tstrct == nullptr ? new type_struct2() :
              // cnode->tstrct;
              if (cnode->tstrct == nullptr) {
                cnode->tstrct = (type_struct2 *)AllocMiscData(
                    lang_stat, sizeof(type_struct2));
            ...
            case node_type::N_FUNC_DECL: {
              ...
              if (cnode->fdecl == nullptr) {
                ...
                cnode->fdecl =
                    (func_decl *)AllocMiscData(lang_stat, sizeof(func_decl));
                ...
                if (!cnode->l->l->l) {
                  decl_exist = DeclareDeclToScopeAndMaybeToFunc(
                      lang_stat, decl_name, &ret_type, scp, n);
                  decl_exist->flags = DECL_NOT_DONE;
                  first_time = true;
                } else if (cnode->l->l->l && overload_strct) {
                  overload_strct->fdecls.emplace_back(cnode->fdecl);
                }
                /*
                else
                {
                        // weird ngl, this probably has to do with templated
                funcs
                        // maybe it doesnt have a overload struct
                        ASSERT(0);
                }
                */
              }
            ...
            if (overload_strct && first_time) {
              ...
              own_std::string new_name = MangleFuncNameWithArgs(
                  lang_stat, ret_type.fdecl, decl_name, 0);
              ...
              FOR_VEC(fdecl, overload_strct->fdecls) {
                ...
                if (f->name == new_name) {
                  REPORT_ERROR(
                      n->t->line, n->t->line_offset,
                      VAR_ARGS("function '%s' has already an overload declared "
                               "here\n%s(%d): %s",
                               overload_strct->name.c_str(),
                               f->from_file->name.c_str(),
                               f->func_node->t->line,
                               GetFileLn(lang_stat, f->func_node->t->line - 1,
                                         f->from_file)));
                  ExitProcess(1);
                }
              }
              overload_strct->fdecls.emplace_back(ret_type.fdecl);
              decl2 *d = DeclareDeclToScopeAndMaybeToFunc(lang_stat, new_name,
                                                          &ret_type, scp, n);
...
#endif
                      */
              colon = FindIdentifier(n->l->t->str, scp, &ret_type,
                                     NM_FND_TP_RETURN_EVEN_IDENT_NOT_DONE);
          ...
          default: {
            ReportMessage(lang_stat, n->t,
                          "unexpected expression in determining var type");
            ASSERT(false)
          } break;
          }
        } break;
      ...
      if (IS_FLAG_OFF(lang_stat->flags, PSR_FLAGS_DONT_DECLARE_VARIABLES) &&
              decl_exist == nullptr ||
          IS_FLAG_ON(lang_stat->flags,
                     PSR_FLAGS_IGNORE_DECL_EXIST_AND_DECLARE)) {
        // adding the new func to the overloaded array
        // we only want to declare functions and structs. We're not interested
        // in its contents
        return DeclareDeclToScopeAndMaybeToFunc(lang_stat, decl_name, &ret_type,
                                                scp, n);
      } else {
        if (decl_exist)
          return decl_exist;
  ...
  case node_type::N_FOR: {
    ...
    if (IsNodeOperator(n, T_IN) && !IsNodeOperator(n->l, T_COLON)) {
      ...
      if (!var_decl) {
        ...
        if (ret_type.type == TYPE_STRUCT) {
          ...
          node *ident_address_of = NewUnOpNode(lang_stat, T_AMPERSAND,
                                               new_node(lang_stat, n->r), n->t);
          node *ident_it_start_call_node = NewTypeNode(
              lang_stat, ident_it_start_node, N_CALL, ident_address_of, n->t);
          node *ident_it_end_call_node = NewTypeNode(
              lang_stat, ident_it_end_node, N_CALL, ident_address_of, n->t);

          node *two_points_bin_nd =
              NewBinOpNode(lang_stat, ident_it_start_call_node, T_TWO_POINTS,
                           ident_it_end_call_node);
        ...
        var_decl = DeclareDeclToScopeAndMaybeToFunc(lang_stat, n->l->t->str,
                                                    &ret_type, for_scope);
    ...
    lang_stat->flags &= ~PSR_FLAGS_INSIDER_FOR_DECL;
    if (n->r != nullptr && !DescendNameFinding(lang_stat, n->r, scp) &&
    scp->parent != nullptr) return nullptr;
            */

    // ASSERT(it.type == enum_type2::TYPE_STRUCT &&
    // it.strct->original_strct->name == "iterator")
  ...
  case node_type::N_IF: {
    /*
    if (CMP_NTYPE_BIN(n->l->l, T_COLON))
    {
            ASSERT(n->l->l->l->type == N_IDENTIFIER);

            node* ident = n->l->l->l;
            node* type = n->l->l->r;

            if (!DescendNameFinding(lang_stat, type, scp))
            {
                    return nullptr;
            }

            // only accepting this kinda of syntax "if new_var : var.IDX ==
    etruct.IDX"

            ASSERT(CMP_NTYPE_BIN(type, T_COND_EQ))

            ret_type = DescendNode(lang_stat, type->r, scp);

            ASSERT(ret_type.type == TYPE_ENUM_IDX_32);

            ASSERT(ret_type.from_enum->type.type == TYPE_STRUCT_TYPE);

            type_struct2* strct = ret_type.from_enum->type.strct;

            ASSERT(IS_FLAG_ON(strct->flags, TP_STRCT_ETRUCT));

            if (!n->l->r->scp)
                    n->l->r->scp = GetScopeFromParent(lang_stat, n, scp);
            scope* new_scp = n->l->r->scp;

            decl2* type_from_enum = strct->scp->vars[ret_type.e_idx + 1];

            decl2* d = NewDecl(lang_stat, ident->t->str, type_from_enum->type);

            node* new_n = NewBinOpNode(lang_stat, type->l, T_POINT,
    NewIdentNode(lang_stat, type_from_enum->name, n->t));
            //new_n = NewBinOpNode(lang_stat, new_node(lang_stat, new_n),
    T_POINT, );


            d->flags |= DECL_FROM_USING;
            d->using_node = new_n;

            new_scp->vars.emplace_back(d);

            auto a = 0;
            memcpy(n->l->l, type->r, sizeof(node));
    }
    */
    // scp = GetScopeFromParent(n, given_scp);
    if (n->l->l != nullptr && !DescendNameFinding(lang_stat, n->l->l, scp) &&
        scp->parent != nullptr)
      return nullptr;
    if (n->l->r != nullptr && !DescendNameFinding(lang_stat, n->l->r, scp) &&
        scp->parent != nullptr)
      return nullptr;
  ...
  case node_type::N_STMNT: {
    ...
    while (true) {
      ...
      if (cur_node->l != nullptr &&
          IS_FLAG_OFF(cur_node->l->flags, NODE_FLAGS_IS_PROCESSED)) {
      ...
      if (cur_node->r != nullptr &&
          IS_FLAG_OFF(cur_node->r->flags, NODE_FLAGS_IS_PROCESSED)) {
  ...
  case N_ARRAY_CONSTRUCTION: {
    ...
    if (!n->scp) {
      n->exprs = (own_std::vector<comma_ret> *)AllocMiscData(
          lang_stat, sizeof(own_std::vector<comma_ret>));
      n->scp = scp;
...
node *CreateTreeArgs(lang_state *lang_stat, own_std::vector<node *> &args,
                     node_type n_tp, tkn_type2 t_tp) {
  node *n_args = nullptr;
  if (args.size() > 1) {
...
node *MakeFuncCallArgs(lang_state *lang_stat, own_std::string fname, node *ref,
                       own_std::vector<node *> &args, token2 *t) {
  node *n_args = new_node(lang_stat, t);

  if (ref) {
    n_args->r = ref->NewTree(lang_stat);
    n_args->t->type = tkn_type2::T_AMPERSAND;
    n_args->t->line = ref->t->line;
  ...
  auto new_n = NewTypeNode(lang_stat, NewIdentNode(lang_stat, fname, t),
                           node_type::N_CALL, n_args, t);

  return new_n;
  // FREE? free(n)
}
void MakeRelPtrDerefFuncCall(lang_state *lang_stat, func_decl *op_func,
                             node *n) {
  if (n->type != N_CALL) {
  ...
  auto new_n =
      NewTypeNode(lang_stat, NewIdentNode(lang_stat, op_func->name, n->t),
                  node_type::N_CALL, new_node(lang_stat, n), n->t);
...
void ModifyFuncDeclToName(lang_state *lang_stat, func_decl *fdecl, node *n,
  ...
  fdecl->name = own_std::string(buffer);
  // weird assert here, but sometimes, for only god knows why, the string gets
  // currupted
  //  data when assigning
...
void MaybeConvertEtructEnumToStructType(type2 &ret_type) {
  if (ret_type.type == TYPE_ENUM) {
    if (ret_type.from_enum->type.type == TYPE_STRUCT_TYPE) {
      auto ptr = ret_type.ptr;

      ret_type = ret_type.from_enum->type;
      ret_type.type = TYPE_STRUCT;
      ret_type.ptr = ptr;
    }
  }
}
void DescendNodeExprOn(lang_state *lang_stat, scope *scp, node *n,
                       type2 *ref_type) {
  if (CMP_NTYPE_BIN(n, T_COMMA)) {
    DescendNodeExprOn(lang_stat, scp, n->l, ref_type);
    DescendNodeExprOn(lang_stat, scp, n->r, ref_type);
  } else {
...
type2 DescendNode(lang_state *lang_stat, node *n, scope *given_scp) {
  ...
  case node_type::N_CAST: {
    ...
    if (lhs_type.type == TYPE_FUNC_DEF) {
      lhs_type.type = TYPE_FUNC_PTR;
      lhs_type.fdecl->flags |= FUNC_DECL_CAST;
    }
    ...
    bool can_rhs_be_ptr =
        ...
        rhs_type.ptr == 0 &&
            (rhs_type.type == TYPE_INT || (GetTypeSize(&rhs_type) == 8));

    if (lhs_type.ptr > 0 && !can_rhs_be_ptr) {

      ReportMessageOne(lang_stat, n->t, "Cannot convert '%s' to a pointer",
                       (char *)TypeToString(rhs_type).c_str());
      ExitProcess(1);
    }
    if (GetTypeSize(&lhs_type) < 8 && rhs_type.ptr > 0) {
      ReportMessageOne(lang_stat, n->t,
                       "pointers cannot be converted to smaller size '%s'",
                       (char *)TypeToString(lhs_type).c_str());
    }
    ...
    case enum_type2::TYPE_U64_TYPE:
      ...
      if (rhs_type.ptr == 0 && rhs_type.type == enum_type2::TYPE_STRUCT) {
        // ReportMessage(lang_stat, n->t,);
        REPORT_ERROR(n->t->line, n->t->line_offset,
                     VAR_ARGS("Non ptr structs cannot be converted to "
                              "integers, in this case \"%s\"\n",
    ...
    case enum_type2::TYPE_STRUCT_TYPE:
      ...
      else if (lhs_type.ptr == 0 && (rhs_type.type != enum_type2::TYPE_U64 ||
                                     rhs_type.type != enum_type2::TYPE_S64)) {
        ReportMessage(lang_stat, n->l->t,
                      "Cast cannot be a plain struct, try a struct ptr");
        ExitProcess(1);
        // ASSERT(false)
      }
      break;
    ...
    if (ret_type.type != TYPE_FUNC_PTR)
      ret_type.type = FromTypeToVarType(ret_type.type);
    if (ret_type.type == TYPE_ENUM && ret_type.e_decl &&
        ret_type.e_decl->type.type == TYPE_ENUM) {
      // HERE()
  ...
  case node_type::N_UNOP: {
    ...
    case tkn_type2::T_AMPERSAND: {
      ...
      if (tp.type == TYPE_INT) {
        ReportMessage(lang_stat, n->t,
                      "Cannot take address of number literals");
        ExitProcess(1);
      }
    ...
    case tkn_type2::T_MUL: {
      ...
      if (ret_type.type == enum_type2::TYPE_STRUCT && ret_type.ptr == 0) {
        auto op_func =
            ret_type.strct->FindOpOverload(lang_stat, overload_op::DEREF_OP, n);
        if (!op_func) {
          REPORT_ERROR(n->t->line, n->t->line_offset,
                       VAR_ARGS("no overload '*' found for struct %s",
                                ret_type.strct->name.c_str()));
          ExitProcess(1);
        }
        ASSERT(op_func)
        /*
...
*/
  ...
  case node_type::N_CALL: {
    ...
    if (scp->fdecl && IS_FLAG_ON(scp->fdecl->flags, FUNC_DECL_COMP) &&
        ...
        fdecl->code != nullptr) {
      ReportMessage(lang_stat, n->t,
                    "Regular funtions cant be called inside comp ones");
      ExitProcess(1);
    }
    ...
    case enum_type2::TYPE_FUNC_PTR: {
      ...
      if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_ASSIGN_SAVED_REGS)) {
        // getting how the max of size of how mny registers should be save for
        // the function
  ...
  case node_type::N_IDENTIFIER: {
    if (n->t->str == "global") {
      ...
      if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_AFTER_TYPE_CHECK)) {
        ...
        int flags = FIND_IDENT_FLAGS_RET_IDENT_EVEN_NOT_DONE *
                    IS_FLAG_ON(n->flags, NODE_FLAGS_RETURN_IDENT_EVEN_NOT_DONE);
        decl = FindIdentifier(n->t->str, scp, &ret_type, flags);
      }
      if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_REPORT_UNDECLARED_IDENTS) &&
          !decl)
        ReportUndeclaredIdentifier(lang_stat, n->t);
...
if(!decl)
{
      ...
      */
      ...
      if (decl->type.type == TYPE_ENUM &&
          ret_type.e_decl->type.type == TYPE_TEMPLATE) {
  ...
  case node_type::N_KEYWORD: {
    ...
    case KW_RETURN: {
      if (n->r != nullptr) {
        ...
        ret_type.type = TYPE_VOID;
      MaybeConvertEtructEnumToStructType(ret_type);

      if (IS_FLAG_OFF(scp->flags, SCOPE_INSIDE_FUNCTION)) {
        REPORT_ERROR(n->t->line, n->t->line_offset,
                     VAR_ARGS("return should only be used inside functions\n"))
        ExitProcess(1);
      }

      // assigning the return type of the function in case it has auto return
      // auto as type
      ...
      if (fdecl != nullptr) {
        if (fdecl->ret_type.type == enum_type2::TYPE_AUTO) {
          ...
          fdecl->ret_type.type = FromTypeToVarType(fdecl->ret_type.type);
        } else if (!CompareTypes(&fdecl->ret_type, &ret_type) &&
                   fdecl->ret_type.type != enum_type2::TYPE_TEMPLATE) {
          ...
          if (!CompareTypes(&fdecl->ret_type, &ret_type, false)) {
            ...
            if (fdecl->ret_type.type == enum_type2::TYPE_VOID) {
              REPORT_ERROR(
                  n->t->line, n->t->line_offset,
                  VAR_ARGS("function \"%s\" didn't expect any return value\n",
                           fdecl->name.c_str()))
            }
            // return types mismatch
              ...
              REPORT_ERROR(
                  n->t->line, n->t->line_offset,
                  VAR_ARGS("return value mismatches with function return "
                           "type.Expected %s, received %s\n\n%s\n...\n",
                           TypeToString(fdecl->ret_type).c_str(),
                           TypeToString(ret_type).c_str(),
                           GetFileLn(lang_stat, fdecl->func_node->t->line - 1)))
            }
          }
          ExitProcess(0);
      ...
      if (ret_type.type == TYPE_BOOL && ret_type.ptr == 0 &&
          n->r->type == N_BINOP) {
        type2 t_bool;
        ...
        auto new_tree =
            CreateTreeArgs(lang_stat, tree_args, N_STMNT, (tkn_type2)0);
  ...
  case node_type::N_INDEX: {
    ...
    // indexing
      ...
      if (n->l->type == N_INDEX) {
        ...
        do {
          ...
          if (i < lhs.dims.size())
            val = lhs.dims[i];
          if (!new_index->l)
            new_index->l = NewBinOpNode(lang_stat, cur->r, T_MUL,
                                        NewIntNode(lang_stat, val, n->t));
          else {
            new_index->r = NewBinOpNode(lang_stat, cur->r, T_MUL,
                                        NewIntNode(lang_stat, val, n->t));
            new_index = NewBinOpNode(lang_stat, new_node(lang_stat, new_index),
                                     T_PLUS, nullptr);
          }
          i--;
          cur = cur->l;
      ...
      bool rhs_can_be_index =
          rhs.type == TYPE_U64 && rhs.ptr == 0 || rhs.type == TYPE_INT;

      if (!rhs_can_be_index) {
        ReportMessageOne(lang_stat, n->r->t,
                         "type '%s' cannot be used as index, use u64",
  ...
  case node_type::N_BINOP: {
    ...
    case tkn_type2::T_COND_OR: {
      ...
      if (n->r->type != N_BINOP &&
          (n->r->type != N_UNOP && n->r->t->type != T_EXCLAMATION)) {
        auto rhs = CreateBinOpRhsBool(lang_stat, new_node(lang_stat, n->r),
                                      KW_TRUE, T_COND_EQ);
        memcpy(n->r, rhs, sizeof(node));
      }
      if (n->l->type != N_BINOP &&
          (n->l->type != N_UNOP && n->l->t->type != T_EXCLAMATION)) {
        auto lhs = CreateBinOpRhsBool(lang_stat, new_node(lang_stat, n->l),
                                      KW_TRUE, T_COND_EQ);
    ...
    case tkn_type2::T_GREATER_EQ: {
      ...
      if (ltp.type == TYPE_STRUCT && rtp.ptr == 0 ||
        ...
        if (ltp.ptr > 0) {
          ReportMessage(lang_stat, n->r->t,
                        "lhs must be a value, but it is a ptr");
          ExitProcess(1);
        }
        ...
        if (rtp.type == TYPE_STRUCT) {
          if (rtp.ptr == 0)
            ...
            ReportMessage(lang_stat, n->r->t,
                          "The struct parameters for a operator overloading "
                          "will already be implicit ptr");
            ExitProcess(1);
          }
        }
        tp_ar.emplace_back(rtp);

        auto op_func = ltp.strct->FindExistingOverload(
            lang_stat, nullptr, (void *)COND_EQ_OP, &tp_ar, false, n);

        if (!op_func) {
          if (IS_FLAG_ON(ltp.strct->flags, TP_STRCT_ETRUCT)) {
            node *point =
                NewBinOpNode(lang_stat, new_node(lang_stat, n->l), T_POINT,
                             NewIdentNode(lang_stat, "type", n->l->t));
            ...
            ReportMessageOne(
                lang_stat, n->r->t,
                "struct '%s' doesn't have an operator '==' overloading",
                (void *)ltp.strct->name.c_str());
        }
        if (IS_FLAG_ON(ltp.strct->flags, TP_STRCT_ETRUCT)) {
          ret_type.type = enum_type2::TYPE_BOOL;
          return ret_type;
        ...
        auto call_nd =
            MakeFuncCallArgs(lang_stat, op_func->name, nullptr, arg_ar, n->t);
        call_nd->t = n->t;
        memcpy(n, call_nd, sizeof(node));
        ...
        if (rtp.type != ltp.type && rtp.ptr == 0 )
        {
                ...
                auto casted = NewTypeNode(lang_stat, lhs_tp, N_CAST,
        new_node(lang_stat, n->r));
    ...
    case tkn_type2::T_PLUS: {
      ...
      if ((ltp.type == TYPE_F32_RAW || ltp.type == TYPE_INT) &&
          rtp.type != ltp.type)
        ltp.type = rtp.type;
      ...
      if (ltp.ptr > 0 || rtp.ptr > 0) {
        ...
        REPORT_ERROR(n->t->line, n->t->line_offset,
                     VAR_ARGS("Operation '%s' cannot be performed on pointers",
                              n->t->ToString().c_str()));
        ExitProcess(1);
      }
      ...
      if (ret_type.type == enum_type2::TYPE_STRUCT && ret_type.ptr == 0) {
        ...
        // func_decl *
        auto op_func = ret_type.strct->FindExistingOverload(
            lang_stat, nullptr, (void *)op, &tp_ar, false, n);
        ASSERT(op_func)
        /*
bool is_correct_ovrld = CompareTypes(&op_func->args[0]->type, &ret_type);
if (!is_correct_ovrld)
...
*/
        ...
        if (op_func->args[1]->type.ptr > 0) {
          ref_nd =
              NewBinOpNode(lang_stat, nullptr, tkn_type2::T_AMPERSAND, sec_arg);
          ref_nd->type = node_type::N_UNOP;
          sec_arg = ref_nd;
        }
      ...
      /*
      if(ltp.IsStrct() && rtp.IsStrct() && GetTypeSize(&ltp) >
      GetTypeSize(&rtp))
      {
      }
      */
      ...
      auto ModifyNd = [](node** nd, type2& ret_type, enum_type2 tp) {
              ...
              {
                      ret_type.f = GetExpressionValT<float>(n->t->type,
      n->l->t->f, n->r->t->f); n->type = node_type::N_FLOAT; n->t->f =
      ret_type.f;
              }
      ...
      if ((ltp_nd == tkn_type2::T_INT && ltp_nd == tkn_type2::T_INT) ||
              (ltp.type == enum_type2::TYPE_F32 && rtp.type ==
      enum_type2::TYPE_F32)) ModifyNd(&n, ret_type, ltp.type);
              */
    ...
    case tkn_type2::T_EQUAL: 
    {
      ...
      if (rtp.type == TYPE_ENUM && rtp.e_decl &&
          rtp.e_decl->type.type == TYPE_ENUM) {
        HERE()
      ...
      if (ltp.type == enum_type2::TYPE_AUTO) {
        ...
        ASSERT(n->l->t->type == tkn_type2::T_COLON &&
               rtp.type != enum_type2::TYPE_AUTO)
        ...
        *ltp.tp = rtp;
        ...
        if (ltp.type == TYPE_VECTOR && ltp.ptr == 0 &&
            n->r->type == N_STRUCT_CONSTRUCTION && n->t->type == T_EQUAL) {
          node *call =
              MakeMemCpyCall(lang_stat, n->l, n->r, n, FLOAT_REG_SIZE_BYTES);
          ...
          auto op_func =
              ltp.strct->FindOpOverload(lang_stat, overload_op::ASSIGN_OP, n);
          bool is_same_strct = rtp.type == enum_type2::TYPE_STRUCT &&
                               rtp.strct->name == ltp.strct->name;
          ...
          */
            ...
            if (!op_func) {
              if (n->r->type != N_QUESTION_MARK && n->t->type == T_EQUAL) {
                ...
                if (!memcpy_func) {
                  ReportMessage(
                      lang_stat, n->t,
                      "Theres no 'memcpy' function to do a copy of a struct");
                }

                node *call =
                    MakeMemCpyCall(lang_stat, n->l, n->r, n, ltp.strct->size);
                memcpy(n, call, sizeof(node));
              }
              /*
              //auto arg1
              REPORT_ERROR(n->l->t->line, n->l->t->line_offset,
                      VAR_ARGS("operator '=' not found whithin struct '%s'",
              ltp.strct->name.c_str())
              );
              ExitProcess(1);
              */
            } else {
              bool is_correct_ovrld =
                  CompareTypes(&op_func->args[1]->type, &rtp);
              if (!is_correct_ovrld) {
                printf("operator '=' overload. ");
                ReportTypeMismatch(lang_stat, n->t, &op_func->args[1]->type,
                                   &rtp);
              }
              ...
              auto new_args =
                  NewBinOpNode(lang_stat, n->l, tkn_type2::T_COMMA, n->r);

              auto new_n = NewTypeNode(
                  lang_stat, NewIdentNode(lang_stat, op_func->name, n->t),
                  node_type::N_CALL, new_args, n->t);
              new_n->t = n->t;
              // FREE? free(n)
              memcpy(n, new_n, sizeof(node));
          ...
          if (rtp.type == ltp.type && n->l->type == N_INDEX && ltp.ptr == 1 &&
              rtp.ptr == 0) {
            node *new_un = NewUnOpNode(lang_stat, T_MUL,
                                       new_node(lang_stat, n->l), n->l->t);
            memcpy(n->l, new_un, sizeof(node));
            ltp.ptr--;
          } else if (rtp.type == ltp.type && n->r->type == N_INDEX &&
                     n->l->type == N_INDEX && ltp.ptr == 1 && rtp.ptr == 1) {
            node *new_un = NewUnOpNode(lang_stat, T_MUL,
                                       new_node(lang_stat, n->l), n->l->t);
            memcpy(n->l, new_un, sizeof(node));
            ltp.ptr--;

            new_un = NewUnOpNode(lang_stat, T_MUL, new_node(lang_stat, n->r),
                                 n->r->t);
          ...
          if (n->r->type != N_QUESTION_MARK) {
            ...
            if (ltp.type == enum_type2::TYPE_STATIC_ARRAY && ltp.ptr == 0) {
              node *call =
                  MakeMemCpyCall(lang_stat, n->l, n->r, n, GetTypeSize(&ltp));
              memcpy(n, call, sizeof(node));
            }
          }
          /*
          // creating a cast for types that are different
          if (ltp.type != TYPE_ENUM && rtp.type != TYPE_STR_LIT && ltp.type !=
          rtp.type)
          {
  ...
  case N_ARRAY_CONSTRUCTION: {
    ...
    if (ret_type.type == TYPE_STATIC_ARRAY_TYPE &&
        ret_type.tp->type == TYPE_FUNC_PTR) {
  ...
  if (IS_FLAG_ON(lang_stat->flags, PSR_FLAGS_HAS_PLUGINS)) {
    ...
    FOR_VEC(it, fdecl->plugins) {
      ...
      {
              auto t_name = own_std::string("$$") + TypeToString(ret_type);
              ASSERT(lang_stat->symbols_offset_on_type_sect.find(t_name) !=
      lang_stat->symbols_offset_on_type_sect.end()); int offset_in_type_sect =
      lang_stat->symbols_offset_on_type_sect[t_name]; offset_in_type_sect =
      lang_stat->type_sect.size() - offset_in_type_sect;

              type_info.ptr = ret_type.ptr;
              type_info.ptr_to_type_data =
      &lang_stat->code_sect[offset_in_type_sect];
      }
...
bool CheckOverloadFunction(lang_state *lang_stat, func_decl *f) {
  ...
  if (f->func_node->l->r &&
      !NameFindingGetType(lang_stat, f->func_node->l->r, f->scp, f->ret_type))
    return false;
  ...
  if (f->args.size() == 0)
    f->args.insert(f->args.end(), f->scp->vars.begin() + f->templates.size(),
                   f->scp->vars.end());
...
func_decl *
type_struct2::FindExistingOverload(lang_state *lang_stat,
                                   own_std::vector<func_overload_strct> *funcs,
                                   void *data, own_std::vector<type2> *tps,
                                   bool search_constructors, node *n) {
  if (search_constructors && funcs->size() > 0) {
    type2 *ret_type = (type2 *)data;
    auto name =
        FuncNameWithTempls(lang_stat, "constr_" + TypeToString(*ret_type), tps);
    ...
    FOR_VEC(f, op_overloads_funcs) {
      ...
      // TYPE_OVERLOAD_FUNCS
        ...
        own_std::string fname = this->name;
        ...
        if (ret) {
          // ASSERT(ret->this_decl->type.type != TYPE_OVERLOADED_FUNCS);
          break;
        }
      }
    }
    if (ret) {
      ...
      // since we dindt find the func, maybe it's a templated one, so we
      // searching in the original struct
      if (original_strct) {
...
func_decl *type_struct2::FindOpOverload(lang_state *lang_stat, overload_op tp,
                                        node *n,
                                        own_std::vector<type2> *tp_ptr) {
  /*
  FOR_VEC(f, op_overloads_funcs)
  {
          if((*f)->type == TYPE_FUNC)
          ...
          else
          {
                  FOR_VEC(ovlrd, (*f)->type.overload_funcs->fdecls)
                  {
                          ...
                          if(IS_FLAG_OFF(func->type.fdecl->flags,
  FUNC_DECL_IS_DONE))
                          {
                                  auto new_func = func;
  ...
  */
  ...
  if (original_strct) {
    ...
    if (found_in_og) {
      own_std::string f_name = this->name + OvrldOpToStr(tp);
      ...
      FOR_VEC(templ, new_func->templates)
      {
              ...
              if (templ->name == cur_strct_var->name)
                      new_func->scp->vars.emplace_back(NewDecl(lang_stat,
      templ->name, *cur_strct_var->type.tp));

              cur_templ++;
      }

      new_func->name = this->name;
      new_func->op_overload = tp;
      */

      this->AddOpOverload(lang_stat, new_func, tp, n->t->line,
                          n->t->line_offset);
      if (!CheckOverloadFunction(lang_stat, new_func))
        return nullptr;
...
unit_file *ThereIsFile(lang_state *lang_stat, own_std::string &dir) {
  ...
  int res =
      GetFullPathName((char *)dir.c_str(), MAX_PATH, name_buffer, nullptr);
  dir = name_buffer;
#endif
...
func_decl *type_struct2::CreateNewOpOverload(lang_state *lang_stat,
                                             func_decl *original,
                                             overload_op tp) {
  // creating overloaded function for this struct
  ...
  new_func->flags =
      FUNC_DECL_IS_OP_OVERLOAD | (original->flags & FUNC_DECL_X64);
  ...
  FOR_VEC(templ, new_func->templates) {
    auto cur_strct_var = this->scp->vars[cur_templ];
    if (templ->name == cur_strct_var->name)
      new_func->scp->AddDecl(
          NewDecl(lang_stat, templ->name, *cur_strct_var->type.tp));
...
decl2 *scope::FindVariableCached(own_std::string &name) {
