#pragma once
#include "sort.h"
template <typename T, typename ValT = int>
void SortRatedStuffDescending(own_std::vector<RatedStuff<T, ValT>>* v)
{
    auto ar = &v->ar;
	int n =  ar->count;
	for (int i = 0; i < n - 1; i++) {
...
void SortRatedStuff(own_std::vector<RatedStuff<T, ValT>> *v)
{
    auto ar = &v->ar;
	int n =  ar->count;
	for (int i = 0; i < n - 1; i++) {
